...
public:
    ...
    explicit DistributedActorSystem(caf::actor_system& actor_system,
    ...
    std::vector<RegisteredActor> find_actors_by_group(const std::string& service_group);
    ...
    size_t broadcast_to_type(ActorType type, T&& message, 
                           bool include_local = true, bool include_remote = true);
...
template<typename T>
bool DistributedActorSystem::send_to_actor(const std::string& actor_name, T&& message) {
...
template<typename T>
size_t DistributedActorSystem::broadcast_to_group(const std::string& service_group, T&& message) {
