...
public:
    ...
    explicit ActorRegistry(caf::actor_system& system, 
    ...
    bool register_actor(const caf::actor& actor, 
    ...
    std::vector<RegisteredActor> find_local_actors_by_type(ActorType type) const;
    ...
    std::vector<RegisteredActor> discover_actors_by_group(const std::string& service_group);
    ...
    void start_heartbeat(std::chrono::seconds interval = std::chrono::seconds(30));
...
private:
    ...
    discovery::ServiceInstance metadata_to_service_instance(const ActorMetadata& metadata,
                                                           const std::string& actor_uri) const;

    /// @brief Convert service instance to registered actor
    RegisteredActor service_instance_to_registered_actor(const discovery::ServiceInstance& instance) const;
...
private:
    ...
    std::unordered_map<std::string, RegisteredActor> local_actors_;  // actor_name -> RegisteredActor
    std::unordered_map<std::string, RegisteredActor> remote_actors_cache_;  // actor_name -> RegisteredActor
