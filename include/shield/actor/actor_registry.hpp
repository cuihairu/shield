...
public:
  ...
  std::vector<RegisteredActor>
  discover_actors_by_group(const std::string &service_group);
  ...
  void
  start_heartbeat(std::chrono::seconds interval = std::chrono::seconds(30));
...
private:
  ...
  metadata_to_service_instance(const ActorMetadata &metadata,
