...
struct LuaMessage {
  ...
  template <class Inspector> friend bool inspect(Inspector &f, LuaMessage &x) {
    return f.object(x).fields(f.field("type", x.type), f.field("data", x.data),
...
struct LuaResponse {
  ...
  // CAF serialization support
  template <class Inspector> friend bool inspect(Inspector &f, LuaResponse &x) {
...
public:
  LuaActor(caf::actor_config &cfg, script::LuaVMPool &lua_vm_pool,
           DistributedActorSystem &actor_system, const std::string &script_path,
...
protected:
  ...
  void
  lua_send_message(const std::string &target_actor, const std::string &msg_type,
