...
struct CoordinatorConfig {
    std::string node_id;
    std::string node_type = "logic";           // Node type: gateway, logic, database, etc.
...
/// Provides a unified interface for initializing and managing all actor-related components
...
public:
    ...
    DistributedActorSystem& get_distributed_system() { return *distributed_system_; }

    /// @brief Get the discovery service
    /// @return Shared pointer to the discovery service
    std::shared_ptr<discovery::IServiceDiscovery> get_discovery_service() { return discovery_service_; }
    ...
    caf::actor spawn_and_register(ActorType type,
                                 const std::string& name,
                                 const std::string& service_group = "",
                                 const std::map<std::string, std::string>& tags = {},
                                 Args&&... args);
...
template<typename ActorImpl, typename... Args>
caf::actor ActorSystemCoordinator::spawn_and_register(ActorType type,
                                                     const std::string& name,
                                                     const std::string& service_group,
                                                     const std::map<std::string, std::string>& tags,
                                                     Args&&... args) {
...
template<typename T>
bool ActorSystemCoordinator::send_to_actor(const std::string& actor_name, T&& message) {
    if (!running_) {
        return false;
    }

    bool result = distributed_system_->send_to_actor(actor_name, std::forward<T>(message));
    if (result) {
...
size_t ActorSystemCoordinator::broadcast_to_type(ActorType type, T&& message) {
    ...
    size_t count = distributed_system_->broadcast_to_type(type, std::forward<T>(message));
