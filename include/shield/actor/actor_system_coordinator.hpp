...
public:
  ...
  caf::actor
  spawn_and_register(ActorType type, const std::string &name,
                     const std::string &service_group = "",
                     const std::map<std::string, std::string> &tags = {},
                     Args &&...args);
  ...
  template <typename T> size_t broadcast_to_type(ActorType type, T &&message);
...
bool ActorSystemCoordinator::send_to_actor(const std::string &actor_name,
  ...
  bool result =
      distributed_system_->send_to_actor(actor_name, std::forward<T>(message));
  if (result) {
...
std::unique_ptr<ActorSystemCoordinator>
make_coordinator_from_config(const shield::core::Config &shield_config);

/// @brief Factory function to create coordinator with default configuration
std::unique_ptr<ActorSystemCoordinator>
make_default_coordinator(const std::string &node_id = "");
