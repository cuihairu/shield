...
struct UdpEndpoint {
    ...
    UdpEndpoint(const boost::asio::ip::udp::endpoint& ep, uint64_t id)
        : endpoint(ep), session_id(id), last_activity(std::chrono::steady_clock::now()) {}
...
class UdpSession {
public:
    using ReceiveCallback = std::function<void(uint64_t session_id, const char* data, size_t length, const boost::asio::ip::udp::endpoint& from)>;
    ...
    void send_to(const boost::asio::ip::udp::endpoint& endpoint, const char* data, size_t length);
    void send_to(uint64_t session_id, const char* data, size_t length);

    // Set callbacks
    void on_receive(ReceiveCallback callback) { m_receive_callback = std::move(callback); }
    void on_timeout(TimeoutCallback callback) { m_timeout_callback = std::move(callback); }

    // Session management
    uint64_t get_or_create_session_id(const boost::asio::ip::udp::endpoint& endpoint);
    void remove_session(uint64_t session_id);
    void cleanup_expired_sessions();

    // Configuration
    void set_session_timeout(std::chrono::seconds timeout) { m_session_timeout = timeout; }
...
private:
    ...
    std::unordered_map<uint64_t, std::shared_ptr<UdpEndpoint>> m_endpoint_sessions;
