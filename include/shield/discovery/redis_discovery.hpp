...
public:
    ...
    RedisServiceDiscovery(const std::string& redis_uri, std::chrono::seconds heartbeat_interval = std::chrono::seconds(5));
...
/// @param redis_uri The Redis connection URI, e.g., "tcp://127.0.0.1:6379".
/// @param heartbeat_interval The interval at which registered services are heartbeated (renewed).
std::unique_ptr<IServiceDiscovery> make_redis_discovery(const std::string& redis_uri, std::chrono::seconds heartbeat_interval = std::chrono::seconds(5));
