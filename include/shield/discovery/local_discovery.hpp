...
/// @brief A local, file-backed implementation of the service discovery
...
private:
    ...
    // Maps a service name to a map of its registered instances,
    // keyed by instance_id.
    std::map<std::string, std::unordered_map<std::string, ServiceInstance>>
        _services;
