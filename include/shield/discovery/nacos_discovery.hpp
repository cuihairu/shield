...
public:
    ...
    /// @param nacos_server_address The Nacos server address (host:port), e.g., "127.0.0.1:8848".
    /// @param heartbeat_interval The interval at which registered services are heartbeated (renewed).
    NacosServiceDiscovery(const std::string& nacos_server_address, std::chrono::seconds heartbeat_interval = std::chrono::seconds(5));
...
private:
    ...
    std::map<std::string, ServiceInstance> _registered_instances; // instance_id -> ServiceInstance

    void _heartbeat_loop();

    // Helper to send HTTP requests
    std::string _send_http_request(http::verb method, const std::string& target, const std::string& body = "") const;
};

/// @brief Factory function to create an instance of NacosServiceDiscovery.
/// @param nacos_server_address The Nacos server address (host:port), e.g., "127.0.0.1:8848".
/// @param heartbeat_interval The interval at which registered services are heartbeated (renewed).
std::unique_ptr<IServiceDiscovery> make_nacos_discovery(const std::string& nacos_server_address, std::chrono::seconds heartbeat_interval = std::chrono::seconds(5));
