...
public:
  static std::unique_ptr<IServiceDiscovery>
  create_service_discovery(const core::Config &config) {
    ...
    if (type == "consul") {
      ...
      return make_redis_discovery(
          redis_uri, std::chrono::seconds(10)); // Default heartbeat interval
    } else if (type == "local") {
      SHIELD_LOG_INFO << "Creating LocalDiscovery client.";
      return make_local_discovery(
          std::chrono::seconds(300),
          config.get_local_discovery_file_path()); // Default cleanup interval
    } else {
