...
public:
    ...
    /// This method should also handle service health checks (e.g., heartbeating).
    /// @param instance The service instance to register.
    /// @return True on success, false otherwise.
    virtual bool register_service(const ServiceInstance& instance, std::optional<std::chrono::seconds> ttl = std::nullopt) = 0;

    /// @brief Deregisters a service instance.
    /// @param instance_id The unique ID of the instance to deregister.
    /// @return True on success, false otherwise.
    virtual bool deregister_service(const std::string& service_name, const std::string& instance_id) = 0;

    /// @brief Queries for a single, healthy instance of a named service.
    /// If multiple instances exist, the implementation should apply a load-balancing
    /// strategy (e.g., random, round-robin) to return one.
    ...
    virtual std::vector<ServiceInstance> query_services_by_metadata(const std::map<std::string, std::string>& metadata_filters) = 0;

    /// @brief Queries for service instances matching structured metadata criteria.
    /// @param service_name The name of the service to query.
    ...
    /// @param environment_filter Optional environment filter (empty means any environment).
