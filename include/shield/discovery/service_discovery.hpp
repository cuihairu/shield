...
public:
  ...
  virtual bool
  register_service(const ServiceInstance &instance,
  ...
  query_service(const std::string &service_name) = 0;

  /// @brief Queries for all healthy instances of a named service.
  /// @param service_name The name of the service to query.
  /// @return A vector of all available service instances.
  virtual std::vector<ServiceInstance>
  query_all_services(const std::string &service_name) = 0;
