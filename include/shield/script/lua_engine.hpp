...
public:
    ...
    std::conditional_t<std::is_void_v<R>, bool, std::optional<R>> call_function(const std::string& name, Args&&... args);
...
std::conditional_t<std::is_void_v<R>, bool, std::optional<R>> LuaEngine::call_function(const std::string& name, Args&&... args) {
    if (!initialized_) {
