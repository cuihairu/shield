...
public:
    ...
    VMHandle acquire_vm(std::chrono::milliseconds timeout = std::chrono::milliseconds{5000});
    void return_vm(std::shared_ptr<PooledLuaVM> vm);
    
    // Script management
    bool preload_script(const std::string& script_path);
    bool preload_script_content(const std::string& script_name, const std::string& content);
