...
public:
    using MessageHandler = std::function<void(uint64_t connection_id, const std::string& message)>;
    ...
    void set_session_provider(std::function<std::shared_ptr<net::Session>(uint64_t)> provider);
    ...
    bool close_connection(uint64_t connection_id, uint16_t code = 1000, const std::string& reason = "");
