...
struct UdpMessage {
    ...
    UdpMessage(uint64_t id, const char* msg_data, size_t length, const boost::asio::ip::udp::endpoint& endpoint)
        : session_id(id)
...
public:
    ...
    explicit UdpProtocolHandler(boost::asio::io_context& io_context, uint16_t port);
    virtual ~UdpProtocolHandler();

    // IProtocolHandler interface
    void handle_data(uint64_t connection_id, const char* data, size_t length) override;
    ...
    void send_to_endpoint(const boost::asio::ip::udp::endpoint& endpoint, const std::string& data);
    
    // Set UDP-specific callbacks
    void set_message_callback(UdpMessageCallback callback) { m_message_callback = std::move(callback); }
    void set_session_timeout_callback(UdpSessionTimeoutCallback callback) { m_session_timeout_callback = std::move(callback); }
...
private:
    void on_udp_receive(uint64_t session_id, const char* data, size_t length, const boost::asio::ip::udp::endpoint& from);
