...
protected:
  template <typename T> std::string serialize_impl(const T &object) {
  ...
  template <typename T> T deserialize_impl(const std::string &json_str) {
    if constexpr (JsonSerializable<T>) {
...
private:
  ...
  template <typename T> std::string serialize_json(const T &object) {
    try {
  ...
  template <typename T> T deserialize_json(const std::string &json_str) {
    try {
