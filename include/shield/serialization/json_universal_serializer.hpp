...
class JsonUniversalSerializer : public UniversalSerializer<SerializationFormat::JSON> {
public:
...
private:
    ...
    std::string serialize_json(const T& object) {
        try {
            if constexpr (requires { to_json(std::declval<nlohmann::json&>(), object); }) {
                // Use ADL to_json
                nlohmann::json j;
                to_json(j, object);
                return j.dump();
                ...
                throw SerializationException("No JSON serialization method available for type: " + 
                                           std::string(typeid(T).name()));
            }
        } catch (const nlohmann::json::exception& e) {
            throw SerializationException("JSON serialization failed: " + std::string(e.what()));
        }
    }
    ...
    T deserialize_json(const std::string& json_str) {
        try {
            ...
            if constexpr (requires(T& obj) { from_json(j, obj); }) {
                ...
                throw SerializationException("No JSON deserialization method available for type: " + 
                                           std::string(typeid(T).name()));
            }
        } catch (const nlohmann::json::exception& e) {
            throw SerializationException("JSON deserialization failed: " + std::string(e.what()));
        }
    }
};
...
template<typename T>
std::string to_json_string(const T& object) requires JsonSerializable<T> {
    return serialize_as<SerializationFormat::JSON>(object);
}

template<typename T>
T from_json_string(const std::string& json_str) requires JsonSerializable<T> {
