...
namespace detail {
template <typename T, SerializationFormat Format> struct has_serialize_method;

template <typename T, SerializationFormat Format> struct has_deserialize_method;
} // namespace detail
...
template <SerializationFormat Format> struct serialization_result_type {
  using type = std::vector<uint8_t>; // Default binary
};

template <> struct serialization_result_type<SerializationFormat::JSON> {
...
template <typename T> constexpr SerializationFormat detect_best_format() {
