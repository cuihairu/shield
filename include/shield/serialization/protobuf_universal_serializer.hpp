...
class ProtobufUniversalSerializer : public UniversalSerializer<SerializationFormat::PROTOBUF> {
public:
...
protected:
    template<typename T>
    std::vector<uint8_t> serialize_impl(const T& object) {
    ...
    template<typename T>
    T deserialize_impl(const std::vector<uint8_t>& data) {
        if constexpr (ProtobufSerializable<T>) {
    ...
    std::vector<uint8_t> serialize_protobuf(const T& message) {
        try {
            if constexpr (std::is_base_of_v<google::protobuf::MessageLite, T>) {
                ...
                throw SerializationException("No Protobuf serialization method available for type: " + 
                                           std::string(typeid(T).name()));
            }
        } catch (const std::exception& e) {
            throw SerializationException("Protobuf serialization failed: " + std::string(e.what()));
        }
    }
        ...
        try {
            if constexpr (std::is_base_of_v<google::protobuf::MessageLite, T>) {
                ...
                return T::ParseFromString(data_str);
                                ...
                                requires(T& obj) { obj.ParseFromString(std::string{}); }) {
                ...
                throw SerializationException("No Protobuf deserialization method available for type: " + 
                                           std::string(typeid(T).name()));
            }
        } catch (const std::exception& e) {
            throw SerializationException("Protobuf deserialization failed: " + std::string(e.what()));
        }
    }
};
...
template<typename T>
std::vector<uint8_t> to_protobuf_bytes(const T& message) requires ProtobufSerializable<T> {
    return serialize_as<SerializationFormat::PROTOBUF>(message);
}

template<typename T>
T from_protobuf_bytes(const std::vector<uint8_t>& data) requires ProtobufSerializable<T> {
    return deserialize_as<SerializationFormat::PROTOBUF, T>(data);
}

// String version convenience functions
template<typename T>
std::string to_protobuf_string(const T& message) requires ProtobufSerializable<T> {
    auto bytes = to_protobuf_bytes(message);
    return std::string(bytes.begin(), bytes.end());
}

template<typename T>
T from_protobuf_string(const std::string& data) requires ProtobufSerializable<T> {
