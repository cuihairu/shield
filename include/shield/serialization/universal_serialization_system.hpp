...
public:
    ...
    template<typename T>
    T deserialize_auto(const std::vector<uint8_t>& data, SerializationFormat format) {
...
private:
    ...
    UniversalSerializationSystem& operator=(const UniversalSerializationSystem&) = delete;
        ...
        switch (format) {
            case SerializationFormat::JSON:
                if constexpr (JsonSerializable<T>) {
                    ...
                    throw SerializationException("Type not compatible with JSON format");
                }
                break;
            case SerializationFormat::PROTOBUF:
                if constexpr (ProtobufSerializable<T>) {
                    ...
                    throw SerializationException("Type not compatible with Protobuf format");
                }
                break;
            case SerializationFormat::MESSAGEPACK:
                if constexpr (MessagePackSerializable<T>) {
                    ...
                    throw SerializationException("Type not compatible with MessagePack format");
                }
                break;
            default:
                throw SerializationException("Unsupported serialization format");
        }
    }
...
void initialize_universal_serialization_system(const SerializationConfig& config = SerializationConfig{});
...
template<typename T>
T deserialize_universal(const std::vector<uint8_t>& data, SerializationFormat format) {
    return UniversalSerializationSystem::instance().deserialize_auto<T>(data, format);
}
