...
public:
  ...
  template <typename T> SerializationFormat get_recommended_format() const {
  ...
  template <typename T> auto serialize_auto(const T &object) {
...
private:
  ...
  UniversalSerializationSystem &
  operator=(const UniversalSerializationSystem &) = delete;
...
template <typename T> auto serialize_universal(const T &object) {
