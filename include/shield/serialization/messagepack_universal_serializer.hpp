...
class MessagePackUniversalSerializer : public UniversalSerializer<SerializationFormat::MESSAGEPACK> {
public:
...
template<typename T>
std::vector<uint8_t> to_messagepack_bytes(const T& object) requires MessagePackSerializable<T> {
    return serialize_as<SerializationFormat::MESSAGEPACK>(object);
}

template<typename T>
T from_messagepack_bytes(const std::vector<uint8_t>& data) requires MessagePackSerializable<T> {
