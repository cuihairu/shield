...
public:
    ...
    static auto serialize_auto(const T& object) {
        ...
        if constexpr (best_format == Format) {
            ...
            static_assert(best_format == Format, "Type not compatible with this serializer");
        }
    }
};
...
private:
    std::unordered_map<SerializationFormat, std::unique_ptr<IUniversalSerializer>> serializers_;
};
...
auto serialize(const T& object) {
    ...
    if constexpr (format == SerializationFormat::JSON) {
        ...
        return static_cast<UniversalSerializer<format>*>(serializer)->serialize(object);
    } else {
    ...
    requires Serializable<T, Format>
{
    auto* serializer = SerializerRegistry::instance().get_serializer(Format);
    if (!serializer) {
        throw SerializationException("No serializer available for specified format");
    }
    ...
    requires Serializable<T, Format>
{
    ...
    return static_cast<UniversalSerializer<Format>*>(serializer)->template deserialize<T>(data);
}
