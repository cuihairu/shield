...
├── auth_plugin.h                ← 认证插件（JWT/OAuth/Steam/微信）⚠️ JWT 为 placeholder
...
| Phase | 内容 | 状态 |
|-------|------|------|
| **P0** | `plugin.h` C ABI + `PluginManager` + DB 插件兼容加载 | ✅ 完成 |
| **P1** | YAML 配置加载 + `shield.plugin.*` Lua API | ✅ 完成 |
| **P2** | REDIS/CACHE/QUEUE/LEADERBOARD/AUTH/METRIC/HEALTH/MATCHMAKING 插件适配器 | ✅ 完成 |
