...
```bash
./build/bin/hello_world --config config/app.json
```

或使用统一入口：

```bash
./build/bin/shield --config config/app.json
```

最终 CLI 契约见 [启动流程运行时语义](runtime-bootstrap.md#cli-契约)。`--config` 可缺省，默认 `config/app.json`；也可以重复传入多个配置文件并按顺序合并。
