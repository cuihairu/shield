...
>
> 本文原描述的是 target 拆分方向。当前 `CMakeLists.txt` 已收敛为 `shield_base`/`shield_log`/`shield_config`/`shield_core`/`shield_plugin`/`shield_transport`/`shield_net`/`shield_lua`/`shield_bootstrap`/`shield` 等 host target；数据后端通过 `plugins/*` 独立共享库提供。旧 discovery/metrics/health/DI 和旧插件 v0 等模块已删除（见 [路线图](roadmap.md)）。本文保留作为 target 结构与依赖约束的参考。
...
- Lua、net、plugin、config、log、bootstrap 都是独立 target。
- discovery、metrics、health、DI、annotations、conditions、旧插件 v0、middleware 旧模块直接删除，不保留兼容 target。
...
6. 新建 `shield_plugin`，承载插件 manifest/catalog、实例生命周期、binding、C ABI 和 `register_lua` 分发；数据后端迁移为独立插件共享库。
7. 新建 `shield_bootstrap`，迁移 Starter 和 `shield::run(argc, argv)`。
8. 删除旧模块 target 和源码：DI、annotations、conditions、events、discovery、旧 core metrics、旧 core health、旧插件 v0、middleware。
...
target_link_libraries(shield_lua
  ...
  PRIVATE shield_log shield_config shield_net shield_plugin ${LUA_LIBRARIES}
)

target_link_libraries(shield_bootstrap
  PUBLIC shield_base
  PRIVATE shield_log shield_config shield_core shield_lua shield_net
)
```
