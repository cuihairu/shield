...
- `tests/cluster/test_cluster_manager.cpp`（18 例：状态机、握手采纳、epoch 失效、心跳恢复、下线清路由、握手超时降级、本地路由表发布/收回、远端表整桶替换与 epoch 校验、占位键早期路由不泄漏）与 `tests/cluster/test_cluster_transport.cpp`（3 例：双 `caf::actor_system` 真实 BASP 握手/保活/下线/同端口重启重连/路由收敛与断连清除）。
...
- **投递**：`set_remote_send_fn()` 无注入方，`send_remote()` 恒返回 `false`；`RemoteSendFn` 的注入点是预留的 transport 接缝（单测已覆盖注入行为）。route cache 命中只完成"名字解析"，跨节点消息投递属 M4。
...
- `shield.cluster.query(node, name)` 可命中对端真实发布的服务名（收敛延迟：握手后 ≤ 一个心跳周期）；对端服务注销后同名查询随下一次心跳失效；peer 降级后同一调用先被可达性检查以 `node_suspect`/`node_offline` 拦下，"节点不可达"与"路由未注册"自此可区分。注意：查询命中只完成名字解析，跨节点消息投递尚未实现。
...
因此：启用 `cluster.node_id` 后，集群会呈现真实的连接/握手/保活/下线视图（健康节点 `online`，故障节点 `offline`），且本地服务发布表在集群内自动同步，`shield.cluster.query` 能解析到远端服务名；但**跨节点投递尚未打通**，查询结果尚不能用于真实调用。未配置 `cluster.node_id` 时整条路径不激活，单节点部署不受影响。远端连接失败的 degrade 契约（见 [官方可选模块契约](optional-modules.md) 的"必须持续暴露 unhealthy 状态"）已满足：故障节点如实暴露 `offline`。
