...
- `tests/cluster/test_cluster_manager.cpp`（18 例：状态机、握手采纳、epoch 失效、心跳恢复、下线清路由、握手超时降级、本地路由表发布/收回、远端表整桶替换与 epoch 校验、占位键早期路由不泄漏）与 `tests/cluster/test_cluster_transport.cpp`（6 例：双 `caf::actor_system` 真实 BASP 握手/保活/下线/同端口重启重连/路由收敛与断连清除；M4 起挂真实 Lua 服务管理器验证跨节点 call/send 全链路、对端停机 fast-fail `node_offline`、慢 callee 超时语义、路由收回 `service_not_found`）。
- **LuaServiceManager 名字变更钩子（M3）**：`set_name_change_notifier(fn)` 在 registry 锁外回调每次已提交的发布变更（spawn 发布、`shield.register`/`unregister`、服务退出收回；空 `service_id` 表示收回）；bootstrap 将其接到 `ClusterManager::on_local_route_changed()`，本节点发布表由此维护。`LuaServiceManager` 本身零 cluster 依赖。
- **route 学习（M3）**：本节点完整发布表随每个心跳捎带广播（`RoutesMsg`），握手完成后立即补发一次（新节点免等首个心跳即可收敛）；空表自然传播"该节点已无任何服务"。接收端要求节点已采纳且 epoch 匹配后整桶替换，stale 实例（旧 epoch）的表被静默丢弃。
- **跨节点投递（M4，`EnvelopeMsg`/`EnvelopeReplyMsg` 数据面）**：
  - **发送侧**：`RemoteSendFn` 由 bootstrap 注入并桥接到 `ClusterTransport::send_envelope()`（按节点查活连接，无连接即 `node_offline`）。`shield.send`/`shield.call`/`shield.call_timeout` 入口解析目标：**本地名字命中优先**（本地命中的名字绝不会被重解释为 `node:service`）；`node:service` 形态且 node 非本节点 → 可达性预检（`node_not_found`/`node_suspect`/`node_offline`/`node_removed` 即失败，`node_offline`/`node_suspect` 带 retryable 标记）+ route cache 解析（未注册 → `service_not_found`）→ 走 envelope。
  - **信封**：`EnvelopeMsg{source_node, service_id, method, args_json, call_session, timeout_ms}`（`call_session=0` 即单向 send，无回包）；载荷保持 JSON 字符串，wire 类型不依赖 Lua/nlohmann。
  - **接收侧（proxied 会话）**：callee 节点把进来的 call 在本地分配一个 **proxied session**——占用与本地 call 同一张 pending_calls 表、`caller_co=nullptr`，复用完全相同的本地派发、完成与超时机制；完成时经 hook 原路回 `EnvelopeReplyMsg` 到 `source_node`。两边 session 空间独立编号，不存在跨节点撞号。
  - **call 语义与本地同形**：caller 协程 yield → envelope → callee 本地 handler 跑完 → `EnvelopeReplyMsg` → caller 协程以 `(ok, values...)` 恢复；超时由 caller 侧既有 CAF 驱动裁决，callee 侧 proxied 会话的销毁兜底为"caller 剩余超时 + 30s slack"（驱动直达 hook，不依赖 caller 存活）。主线程同步 `call_with_session()` 走同一套 pending_sync_calls 阻塞语义。
  - **降级语义**：peer 非 Online → 预检立即失败（错误码复用 `node_offline`/`node_suspect`，retryable=true，与本地 `service_not_found` 可区分）；对端断连后 route cache 清空 → `service_not_found`。远程失败（对端派发不存在的服务等）以对端错误码经 `EnvelopeReplyMsg` 原样透传。
