...
| `shield_config` | JSON 加载、环境变量展开、schema 校验 | `shield.config(path)` 读取 | service registry、Lua VM |
...
插件系统 v1 的 `shield.plugin.*` 仅作为只读 introspection API，查询 package、instance 和 binding 状态；它不恢复旧 Lua 插件执行模型。
