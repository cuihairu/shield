...
- 不应把 `auth.jwt`、`oauth`、`wechat`、`steam`、业务 RPC client 这类上层策略做成 core 的中心能力。
...
```text
应用层 / 插件层
  -> auth / jwt / oauth / webhook / redis client / 自定义 RPC client
...
当前仓库已有 `auth.jwt` 插件，但它更像一个具体业务能力，而不是基础组件。
...
但插件设计应优先复用基础组件，而不是重复发明原语。

例如：

- `auth.jwt` 如果长期保留，更合理的定位应是“上层 provider / 示例实现”
- 它的密码学原语应建立在 `shield.crypto` 之上
...
4. 把 `auth.jwt` 从“看起来像 core 能力”调整为“基于 primitives 的上层 provider”
