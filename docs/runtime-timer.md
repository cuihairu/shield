...
`shield.fork` 将一个当前 service 归属的后台 task 投递到 owning service actor（经 `fork_task_atom` 在 actor 调度点执行），不创建新 service。

```lua
local task_id = shield.fork(function()
  shield.send("player", "tick")
end)
```
...
`shield.fork` 当前返回 numeric task id。

```lua
local task_id = shield.fork(fn)
```

当前契约不提供 task cancel、status 或 join，避免引入额外等待图和死锁语义。需要结果时应使用 service `call` 或业务 channel。
