...
如果讨论焦点是 `xmldef` 如何衔接 descriptor、Unity/U3D generator plugin、服务端 Lua binding 和 runtime codec，请优先阅读专项草案 [Xmldef Toolchain Design](xmldef-toolchain-design.md)。若需要更硬的实现契约，再继续读 [Xmldef Descriptor Spec](xmldef-descriptor-spec.md)、[Xmldef Unity Generator Spec](xmldef-unity-generator-spec.md)、[Xmldef Phase 1 Implementation Plan](xmldef-phase1-implementation.md) 和 [Xmldef Compiler / Runtime MVP](xmldef-compiler-runtime-mvp.md)。本文仍是更大的 schema/descriptor 总纲。
