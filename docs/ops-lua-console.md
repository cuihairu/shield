...
- `memory`：owner 线程一次 dispatch 内联 GC 采样与 retainers 摘要（同 `refs` 的 fork task + 2s bounded wait 模型）：`total_bytes`/`memory_kb`（`GCCOUNT`+`GCCOUNTB` 精确采样）、`gc`（collector 运行标志、模式 incremental|generational、六个 collector 参数 minormul/majorminor/minormajor/pause/stepmul/stepsize）、`retainers`（从 module table 出发 depth 4 / 20000 节点一次遍历的头：nodes_visited/truncated/top_tables）。owner 忙时以 `memory dispatch timeout (owner busy)` 报错，不挂起 console。
- `refs`：从 service module table 出发，按深度/数量限制遍历对象图，输出 top tables。
- `coroutines`：owner 线程一次 dispatch 枚举 live 协程注册表（同 `memory` 的 fork task + 2s bounded wait 模型）：`total`、`by_status`（suspended/finished/error/other，按 owner 线程 `lua_status` 分类——协程恒由 owner 线程 resume，枚举期间无并发驱动）、`entries`（每协程：`status`、`origin`（首驱来源 handler/fork/timer/client_rpc）、`resumes`（C++ resume 计数）、`last_resume`（最近 C++ resume 来源 dispatch/call-response/call-timeout；Lua 自驱动 wrap 协程的 resume 无 C++ 观测点，如实停在末次 C++ 驱动）、`age_ms`、`driving`、`waiting_call`（服务中的 call session——callee 侧或 caller 侧，无则 null）），按最近 resume 排序保留 32 条并附 `truncated`。owner 忙时以 `coroutines dispatch timeout (owner busy)` 报错。
