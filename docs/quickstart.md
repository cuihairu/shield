...
- `config/app.json` 已收敛为 Phase 1 最小配置；配置错误会在启动期 fail fast。
...
./build-msvc-examples/bin/hello_world --config examples/hello_world/config/app.json
./build-msvc-examples/bin/hello_world --check-config --config examples/hello_world/config/app.json
```
