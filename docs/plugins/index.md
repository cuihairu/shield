...
Shield 通过插件系统 v1 提供后端能力。每个插件是一个独立的 shared library，附带 manifest 文件描述它提供什么接口、需要什么配置。本节为每个官方插件提供单独的参考页：包含 manifest 字段、配置 schema、ABI 接口、Lua API（如果有）和使用示例。
...
const shield_plugin_abi_v1* shield_plugin_get_v1(void) {
    static const shield_plugin_abi_v1 abi = {
        ...
        "database.my-engine",   // 必须与 manifest 中的 "id" 完全一致
        "1.0.0",
        my_create,
    };
    return &abi;
...
- **Metadata-first**：所有信息（接口、依赖、配置、文档 URL）声明在 manifest
- **C ABI 稳定**：跨编译器、跨版本兼容
