...
> 状态：历史材料，非当前权威口径。
>
> 本文记录的是一次中间重构尝试的目标与阶段性判断，不能作为“当前源码已经完成重构”的证明。当前权威口径以 [最终架构总纲](architecture.md)、[Lua API 契约](lua-api.md)、[运行时语义决策稿](runtime-semantics.md)、[配置运行时语义](runtime-config.md)、[官方可选模块契约](optional-modules.md) 和 [重构路线图](roadmap.md) 为准。
>
