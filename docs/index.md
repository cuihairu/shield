...
features:
  ...
  - title: 插件系统 v1
    details: 基于 YAML manifest 的 metadata-first 扩展机制，稳定 C ABI 提供数据库/缓存/队列/排行榜/认证等后端，按需加载、显式依赖注入。
  - title: 核心 runtime 已落地
    details: 单节点 Lua service、消息、定时器、TCP gateway、原始 DB/Redis、YAML 配置、插件系统 v1 已实现；cluster/global/player/server/ops 等官方可选模块分阶段推进。
  - title: API 契约优先
    details: Lua API 以独立契约和测试矩阵为准，示例只作为用户参考。
  - title: 运维分层
    details: shield_ops 独立承载 metrics、health、console、profile，与 shield_core 解耦。
footer: Apache License 2.0 | Core runtime landed, optional modules in progress
---

## 关于 Shield

Shield 是一个受 Skynet 启发的游戏服务器运行时，采用 Actor 模型和 Lua 优先的设计理念。核心 runtime（单节点 Lua service、消息、定时器、TCP gateway、原始 DB/Redis、YAML 配置、插件系统 v1）已经落地；cluster/global/player/server/ops 等官方可选模块按 roadmap 分阶段推进。
...
核心 runtime 已经落地：单节点 Lua service、消息与定时器、TCP gateway、原始 DB/Redis、YAML 配置、插件系统 v1。`shield_cluster`、`shield_global`、`shield_player`、`shield_server`、`shield_ops` 等官方可选模块按 [路线图](/roadmap) 分阶段推进。

部分目录下仍有历史残留代码（旧 DI / annotations / discovery 等）等待清理；新增功能以 [插件系统 v1](/plugin-system) 和官方可选模块为唯一扩展路径。

查看[路线图](/roadmap)了解分阶段计划和当前范围。
