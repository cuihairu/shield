...
- `shield_ops`：Prometheus 指标、健康检查、HTTP/console 管理端点、profile。（P0 已落地 `/ops/health`（verdict 映射 503）+ `/ops/metrics` + `/ops/services/:name` 含 per-service 流量/uptime/timers/pending_calls/pending_tasks/coroutines/memory_kb 统计（L1 全量）；`/ops/profile` 与 L2 inspect 留后续）
