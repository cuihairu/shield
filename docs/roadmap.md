...
- `shield_ops`：Prometheus 指标、健康检查、HTTP/console 管理端点、profile。（P0 已落地 `/ops/health`（verdict 映射 503）+ `/ops/metrics` + `/ops/services/:name` 含 per-service 流量/uptime/timers 统计；`/ops/profile`、coroutines 类统计留后续）
