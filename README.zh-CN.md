...
- DI/IoC 容器或基于注解的装配。
- ORM 或企业数据框架。
- 独立于 Actor 消息的跨服务事件总线或生命周期事件总线。
- 作为框架策略层的中间件链。
- Prometheus、健康检查注册或插件系统作为核心运行时特性。

这些能力可能由用户构建或稍后作为可选扩展进行评估。它们不是当前核心契约的一部分。

`shield.event` 只是单个 Lua service / 单个 Lua VM 内部的轻量解耦工具，不进入 mailbox、不序列化、不跨 service/VM/node，也不用于 application lifecycle 事件。
...
| `shield_player` | PlayerSession、PlayerRef、登录/登出、断线重连、玩家 ready 生命周期 |
| `shield_server` | 服务器状态、维护模式、关服编排 |
