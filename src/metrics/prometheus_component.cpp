...
SystemMetricsCollector::SystemMetricsCollector(std::shared_ptr<prometheus::Registry> registry) {
    // CPU usage gauge
...
double SystemMetricsCollector::get_cpu_usage() {
    ...
    if (host_statistics(mach_host_self(), HOST_CPU_LOAD_INFO, 
        ...
        return static_cast<double>(cpuinfo.cpu_ticks[CPU_STATE_USER] + 
                                 cpuinfo.cpu_ticks[CPU_STATE_SYSTEM]) / totalTicks * 100.0;
    }
#elif __linux__
...
NetworkMetricsCollector::NetworkMetricsCollector(std::shared_ptr<prometheus::Registry> registry) {
...
void NetworkMetricsCollector::collect() {
    // Network metrics are updated via method calls, no periodic collection needed
}
...
GameMetricsCollector::GameMetricsCollector(std::shared_ptr<prometheus::Registry> registry) {
    ...
    messages_processed_family_ = &prometheus::BuildCounter()
...
void PrometheusComponent::on_init() {
    try {
        ...
        // Load configuration
        try {
            listen_address_ = config.get<std::string>("prometheus.listen_address");
        } catch (...) {
            // Use default
        }
        ...
        try {
            collection_interval_ = std::chrono::seconds(config.get<int>("prometheus.collection_interval"));
        } catch (...) {
            // Use default
        }
        ...
        try {
            pushgateway_url_ = config.get<std::string>("prometheus.pushgateway_url");
        ...
        network_collector_ = std::make_shared<NetworkMetricsCollector>(registry_);
        ...
        std::cout << "Prometheus component initialized (metrics disabled - prometheus-cpp not available)" << std::endl;
#endif
                  
    } catch (const std::exception& e) {
        std::cerr << "Failed to initialize Prometheus component: " << e.what() << std::endl;
        throw;
    }
}
...
void PrometheusComponent::on_start() {
    try {
        ...
        if (enable_exposer_) {
            exposer_ = std::make_unique<prometheus::Exposer>(listen_address_ + ":" + std::to_string(listen_port_));
            exposer_->RegisterCollectable(registry_);
            std::cout << "Prometheus metrics exposed on http://" << listen_address_ 
                      << ":" << listen_port_ << "/metrics" << std::endl;
        }
        
        // Setup push gateway if enabled
        if (enable_pushgateway_ && !pushgateway_url_.empty()) {
            gateway_ = std::make_unique<prometheus::Gateway>(pushgateway_url_, job_name_);
            gateway_->RegisterCollectable(registry_);
            std::cout << "Prometheus push gateway configured: " << pushgateway_url_ << std::endl;
        }
#endif
        
        // Start collection thread
        running_ = true;
        collection_thread_ = std::thread(&PrometheusComponent::collection_loop, this);
        
        std::cout << "Prometheus component started" << std::endl;
        
    } catch (const std::exception& e) {
        std::cerr << "Failed to start Prometheus component: " << e.what() << std::endl;
        throw;
    }
}
...
void PrometheusComponent::on_stop() {
    ...
    if (gateway_) {
        try {
            ...
            std::cerr << "Failed to push final metrics: " << e.what() << std::endl;
        }
        gateway_.reset();
    }
...
void PrometheusComponent::add_collector(std::shared_ptr<MetricsCollector> collector) {
...
void PrometheusComponent::collection_loop() {
    while (running_) {
        try {
            ...
            std::cerr << "Error during metrics collection: " << e.what() << std::endl;
        }
        
        // Sleep for collection interval
        auto start = std::chrono::steady_clock::now();
        while (running_ && std::chrono::steady_clock::now() - start < collection_interval_) {
