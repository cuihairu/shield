...
UdpProtocolHandler::UdpProtocolHandler(boost::asio::io_context& io_context, uint16_t port)
    : m_udp_session(std::make_unique<shield::net::UdpSession>(io_context, port)) {
    
    // Set up UDP session callbacks
    m_udp_session->on_receive(
        [this](uint64_t session_id, const char* data, size_t length, const boost::asio::ip::udp::endpoint& from) {
...
void UdpProtocolHandler::handle_data(uint64_t connection_id, const char* data, size_t length) {
    // This method is primarily for compatibility with the IProtocolHandler interface
    // In UDP context, data handling is done through the UDP-specific callback
...
bool UdpProtocolHandler::send_data(uint64_t connection_id, const std::string& data) {
    if (!m_udp_session) {
...
void UdpProtocolHandler::send_to_endpoint(const boost::asio::ip::udp::endpoint& endpoint, const std::string& data) {
    if (!m_udp_session) {
        SHIELD_LOG_ERROR << "Cannot send data: UdpSession is null";
        return;
    }
...
void UdpProtocolHandler::on_udp_receive(uint64_t session_id, const char* data, size_t length, const boost::asio::ip::udp::endpoint& from) {
