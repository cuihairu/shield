...
void HttpRouter::add_route(const std::string& method, const std::string& path_pattern, RouteHandler handler) {
    Route route;
    route.method = method;
...
HttpResponse HttpRouter::route_request(const HttpRequest& request) {
    ...
    for (const auto& route : routes_) {
        if (route.method == request.method && std::regex_match(request.path, route.path_regex)) {
    ...
    response.body = R"({"error": "Not Found", "path": ")" + request.path + R"("})";
    return response;
}
...
void HttpProtocolHandler::handle_data(uint64_t connection_id, const char* data, size_t length) {
...
bool HttpProtocolHandler::send_data(uint64_t connection_id, const std::string& data) {
...
void HttpProtocolHandler::set_session_provider(std::function<std::shared_ptr<net::Session>(uint64_t)> provider) {
    session_provider_ = std::move(provider);
}

HttpRequest HttpProtocolHandler::parse_http_request(const std::string& raw_request, uint64_t connection_id) {
    HttpRequest request;
...
std::string HttpProtocolHandler::format_http_response(const HttpResponse& response) {
...
bool HttpProtocolHandler::is_complete_request(const std::string& data) {
    ...
    if (content_length_it != headers.end()) {
        size_t content_length = std::stoul(content_length_it->second);
        size_t total_expected = header_end + 4 + content_length; // +4 for "\r\n\r\n"
