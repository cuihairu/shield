...
YAML::Node merge_yaml_nodes(const YAML::Node& base, const YAML::Node& override) {
...
void Config::load_with_profile(const std::string& profile) {
    ...
    try {
        ...
        SHIELD_LOG_INFO << "Loaded base configuration: " << ConfigPaths::DEFAULT_CONFIG_FILE;
    } catch (const YAML::Exception& e) {
        throw std::runtime_error("Failed to load base config: " + std::string(e.what()));
    }
    
    // 2. Load profile-specific config if specified
    if (!profile.empty()) {
        std::string profile_file = ConfigPaths::get_profile_config_file(profile);
        
        if (!std::filesystem::exists(profile_file)) {
            throw std::runtime_error("Profile config file not found: " + profile_file);
        }
        ...
        try {
            ...
            throw std::runtime_error("Failed to load profile config: " + profile_file + ", Error: " + e.what());
        }
    }
    
    config_ = base_config;
    SHIELD_LOG_INFO << "Configuration loaded successfully" << (profile.empty() ? "" : " with profile: " + profile);
}

// Helper function to convert YAML node to sol2 object (TODO: implement when needed)
/*
...
void Config::bind_section_to_lua(sol::state& lua, const std::string& section_key, const std::string& lua_table_name) {
    // Implementation commented out for now
}
*/
