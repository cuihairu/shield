...
RootCommand::RootCommand() 
    : Command("shield", "Shield Game Framework - A high-performance game server") {
    
    set_long_description("Shield is a modern game server framework built with C++20. "
        ...
        .set_example("  shield server --config config/prod.yaml\\n"
                    "  shield cli --url http://localhost:8080\\n"
                    "  shield test --suite integration");
    
    // Add global flags
    add_flag("config", "Global configuration file", ConfigPaths::DEFAULT_CONFIG_FILE);
    add_bool_flag("version", "Show version information", false);
