...
UdpReactor::UdpReactor(uint16_t port, size_t num_worker_threads)
    ...
    SHIELD_LOG_INFO << "UdpReactor created for port " << port << " with " << num_worker_threads << " worker threads";
}
...
void UdpReactor::start() {
    ...
    try {
        ...
        if (m_handler_creator) {
            ...
            m_handler = std::make_unique<shield::protocol::UdpProtocolHandler>(m_io_context, m_port);
        }
        ...
        for (size_t i = 0; i < m_num_worker_threads; ++i) {
            m_worker_threads.emplace_back([this, i]() {
                SHIELD_LOG_DEBUG << "UDP reactor worker thread " << i << " starting";
                
                while (m_running.load()) {
                    try {
                        m_io_context.run();
                        
                        // If run() exits and we're still supposed to be running, restart it
                        if (m_running.load() && m_io_context.stopped()) {
                            m_io_context.restart();
                        }
                    } catch (const std::exception& e) {
                        SHIELD_LOG_ERROR << "UDP reactor worker " << i << " exception: " << e.what();
                    }
                }
                
                SHIELD_LOG_DEBUG << "UDP reactor worker thread " << i << " stopping";
            });
        }
        
        SHIELD_LOG_INFO << "UdpReactor started on port " << m_port << " with " << m_num_worker_threads << " worker threads";
