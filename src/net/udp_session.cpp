...
void UdpSession::send_to(const boost::asio::ip::udp::endpoint& endpoint, const char* data, size_t length) {
    if (!m_running) {
    ...
    m_socket.async_send_to(
        ...
        [this, endpoint](boost::system::error_code ec, std::size_t bytes_sent) {
            if (ec) {
                SHIELD_LOG_ERROR << "UDP send failed to " << endpoint.address().to_string() << ": " << ec.message();
            } else {
                SHIELD_LOG_DEBUG << "UDP sent " << bytes_sent << " bytes to " << endpoint.address().to_string();
            }
        }
    );
}
...
void UdpSession::send_to(uint64_t session_id, const char* data, size_t length) {
    ...
    if (it != m_endpoint_sessions.end()) {
        ...
        SHIELD_LOG_WARN << "Attempt to send to unknown session ID: " << session_id;
    }
}

uint64_t UdpSession::get_or_create_session_id(const boost::asio::ip::udp::endpoint& endpoint) {
    std::string endpoint_key = endpoint.address().to_string() + ":" + std::to_string(endpoint.port());
    ...
    SHIELD_LOG_DEBUG << "Created new UDP session " << session_id << " for " << endpoint.address().to_string();
...
void UdpSession::cleanup_expired_sessions() {
    ...
    if (!expired_sessions.empty()) {
        SHIELD_LOG_DEBUG << "Cleaned up " << expired_sessions.size() << " expired UDP sessions";
    }
}
...
void UdpSession::do_receive() {
    ...
    m_socket.async_receive_from(
        boost::asio::buffer(m_receive_buffer, max_length),
        m_sender_endpoint,
        [this](boost::system::error_code ec, std::size_t bytes_received) {
            if (!ec && bytes_received > 0) {
                uint64_t session_id = get_or_create_session_id(m_sender_endpoint);
                
                if (m_receive_callback) {
                    m_receive_callback(session_id, m_receive_buffer, bytes_received, m_sender_endpoint);
                }
                
                SHIELD_LOG_DEBUG << "UDP received " << bytes_received << " bytes from " << m_sender_endpoint.address().to_string() << " (session " << session_id << ")";
