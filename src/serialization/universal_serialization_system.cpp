...
void UniversalSerializationSystem::initialize(const SerializationConfig& config) {
    if (initialized_) {
    ...
    try {
        ...
        if (config.enable_protobuf) {
            registry.register_serializer(SerializationFormat::PROTOBUF, 
        ...
        if (config.enable_messagepack) {
            registry.register_serializer(SerializationFormat::MESSAGEPACK, 
        ...
        SHIELD_LOG_ERROR << "Failed to initialize serialization system: " << e.what();
        throw SerializationException("Serialization system initialization failed: " + std::string(e.what()));
    }
}
...
std::string UniversalSerializationSystem::get_system_info() const {
    ...
    switch (config_.default_format) {
        case SerializationFormat::JSON: oss << "JSON"; break;
        case SerializationFormat::PROTOBUF: oss << "Protobuf"; break;
        case SerializationFormat::MESSAGEPACK: oss << "MessagePack"; break;
        case SerializationFormat::SPROTO: oss << "sproto"; break;
        default: oss << "Unknown"; break;
    }
    oss << "\n";
    
    oss << "  Auto Format Detection: " << (config_.enable_auto_format_detection ? "Enabled" : "Disabled") << "\n";
...
std::vector<std::string> UniversalSerializationSystem::get_available_formats() const {
...
void initialize_universal_serialization_system(const SerializationConfig& config) {
