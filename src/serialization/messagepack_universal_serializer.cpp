...
public:
    ...
    using DeserializeFunc =
        std::function<void(const std::vector<uint8_t>&, void*)>;
    ...
    void register_type() {
        if constexpr (MessagePackSerializable<T>) {
            ...
            serialize_funcs_[type_idx] =
                [](const void* obj) -> std::vector<uint8_t> {
            ...
            deserialize_funcs_[type_idx] = [](const std::vector<uint8_t>& data,
                                              void* obj) {
    ...
    std::vector<uint8_t> serialize(const void* object,
                                   const std::type_info& type) {
        std::type_index type_idx(type);
        auto it = serialize_funcs_.find(type_idx);
        if (it == serialize_funcs_.end()) {
            throw SerializationException(
                "Type not registered for MessagePack serialization: " +
                std::string(type.name()));
        }
        return it->second(object);
    }

    void deserialize(const std::vector<uint8_t>& data, void* object,
                     const std::type_info& type) {
        std::type_index type_idx(type);
        auto it = deserialize_funcs_.find(type_idx);
        if (it == deserialize_funcs_.end()) {
            throw SerializationException(
                "Type not registered for MessagePack deserialization: " +
                std::string(type.name()));
        }
...
std::vector<uint8_t> MessagePackUniversalSerializer::serialize_bytes(
    const void* object, const std::type_info& type) {
    return MessagePackTypeRegistry::instance().serialize(object, type);
}

void MessagePackUniversalSerializer::deserialize_bytes(
    const std::vector<uint8_t>& data, void* object,
    const std::type_info& type) {
