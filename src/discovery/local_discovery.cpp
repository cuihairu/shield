...
LocalServiceDiscovery::LocalServiceDiscovery(std::chrono::seconds cleanup_interval, const std::string& persistence_file_path)
...
bool LocalServiceDiscovery::register_service(const ServiceInstance& instance_param, std::optional<std::chrono::seconds> ttl) {
    if (instance_param.service_name.empty() || instance_param.instance_id.empty()) {
    ...
    if (ttl) {
        instance.expiration_time = std::chrono::steady_clock::now() + ttl.value();
    } else {
        // Set to a very distant future for services that don't expire automatically
...
bool LocalServiceDiscovery::deregister_service(const std::string& service_name, const std::string& instance_id) {
    ...
    if (it != _services.end()) {
        ...
        if (it->second.empty()) {
            _services.erase(it); // Remove service_name entry if no instances left
        }
    }
    return true;
}

std::optional<ServiceInstance> LocalServiceDiscovery::query_service(const std::string& service_name) {
...
std::vector<ServiceInstance> LocalServiceDiscovery::query_all_services(const std::string& service_name) {
    std::shared_lock<std::shared_mutex> lock(_mutex);
    auto it = _services.find(service_name);
    if (it == _services.end()) {
        return {};
...
std::vector<ServiceInstance> LocalServiceDiscovery::query_services_by_metadata(const std::map<std::string, std::string>& metadata_filters) {
    ...
    for (const auto& service_pair : _services) {
        for (const auto& instance_pair : service_pair.second) {
            ...
            for (const auto& filter_pair : metadata_filters) {
                if (!instance.metadata.matches_filters({{filter_pair.first, filter_pair.second}})) {
...
void LocalServiceDiscovery::_cleanup_loop() {
    while (_running_cleanup) {
        ...
        _cleanup_cv.wait_for(lock, _cleanup_interval, [this] { return !_running_cleanup; });
        ...
        for (auto service_it = _services.begin(); service_it != _services.end(); ) {
            auto& instances_map = service_it->second;
            for (auto instance_it = instances_map.begin(); instance_it != instances_map.end(); ) {
...
void LocalServiceDiscovery::_save_services_to_file() {
    std::shared_lock<std::shared_mutex> lock(_mutex); // Read lock for _services
    nlohmann::json j;
    for (const auto& service_pair : _services) {
    ...
    try {
        ...
        SHIELD_LOG_ERROR << "Failed to create directories for persistence file " << file_path.parent_path() << ": " << e.what();
        return;
    }
    ...
    if (ofs.is_open()) {
        ...
        try {
            ...
            SHIELD_LOG_ERROR << "Failed to rename temporary file " << temp_file_path << " to " << file_path << ": " << e.what();
        }
    } else {
        SHIELD_LOG_ERROR << "Could not open temporary file " << temp_file_path << " for writing.";
    }
}

void LocalServiceDiscovery::_load_services_from_file() {
    std::unique_lock<std::shared_mutex> lock(_mutex); // Write lock for _services
        ...
        try {
            ...
            for (auto it = j.begin(); it != j.end(); ++it) {
                ...
                for (const auto& instance_json : it.value()) {
                    ServiceInstance instance = instance_json.get<ServiceInstance>();
                    _services[service_name][instance.instance_id] = instance;
                }
            }
        } catch (const nlohmann::json::parse_error& e) {
            SHIELD_LOG_ERROR << "JSON parse error when loading persistence file " << _persistence_file_path << ": " << e.what();
        } catch (const nlohmann::json::exception& e) {
            SHIELD_LOG_ERROR << "JSON exception when loading persistence file " << _persistence_file_path << ": " << e.what();
        }
        ifs.close();
    } else {
        SHIELD_LOG_WARN << "Persistence file not found or could not be opened for reading: " << _persistence_file_path << ". Starting with empty service registry.";
    }
}

std::unique_ptr<IServiceDiscovery> make_local_discovery(std::chrono::seconds cleanup_interval, const std::string& persistence_file_path) {
    return std::make_unique<LocalServiceDiscovery>(cleanup_interval, persistence_file_path);
}
