...
bool ServiceMetadata::matches_filters(const std::map<std::string, std::string>& filters) const {
...
ServiceMetadata ServiceMetadata::from_json(const std::string& json_str) {
    ...
    try {
        ...
        if (j.contains("custom_attributes")) {
            metadata.custom_attributes = j.at("custom_attributes").get<std::map<std::string, std::string>>();
        }
...
void to_json(nlohmann::json& j, const ServiceInstance& p) {
    ...
    // Only include expiration_time if it's not max (i.e., has a meaningful value)
    if (p.expiration_time != std::chrono::steady_clock::time_point::max()) {
        j["expiration_time"] = std::chrono::duration_cast<std::chrono::milliseconds>(
            p.expiration_time.time_since_epoch()).count();
    }
}
...
void from_json(const nlohmann::json& j, ServiceInstance& p) {
    ...
    if (j.contains("metadata")) {
        p.metadata = ServiceMetadata::from_json(j.at("metadata").get<std::string>());
    }
    
    if (j.contains("expiration_time")) {
        auto duration = std::chrono::milliseconds(j.at("expiration_time").get<int64_t>());
