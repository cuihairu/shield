...
NacosServiceDiscovery::NacosServiceDiscovery(
    ...
    : _heartbeat_interval(heartbeat_interval), _running_heartbeat(true) {
    ...
    _heartbeat_thread =
        std::thread(&NacosServiceDiscovery::_heartbeat_loop, this);
}
...
std::string NacosServiceDiscovery::_send_http_request(
    ...
    if (ec == net::error::eof) {
        // Rationale: Shutdown causes an error::eof when the other end is
        // closed. For this example, we ignore that.
        ec = {};
    }
    if (ec) {
...
bool NacosServiceDiscovery::register_service(
    ...
    std::stringstream ss;
    ss << "serviceName=" << instance.service_name << "&ip="
       << instance.address.substr(
       ...
       << "&instanceId=" << instance.instance_id << "&weight=1.0"
       << "&enabled=true"
       << "&healthy=true"
       << "&ephemeral=true";  // Ephemeral instances are automatically removed
                              // if heartbeat stops

    // Nacos does not directly support TTL for instances in the same way etcd
    // does. Instead, it relies on the client sending heartbeats for ephemeral
    // instances. The 'ttl' parameter here can be used to determine the
    // heartbeat interval for this instance. For now, we'll just use the class's
...
bool NacosServiceDiscovery::deregister_service(const std::string& service_name,
    ...
    std::stringstream ss;
    ss << "serviceName=" << service_name << "&instanceId=" << instance_id
       << "&ephemeral=true";
    ...
    std::string response =
        _send_http_request(http::verb::delete_, target, body);
...
std::optional<ServiceInstance> NacosServiceDiscovery::query_service(
        ...
        if (j.contains("hosts") && j["hosts"].is_array()) {
            ...
            for (const auto& host : j["hosts"]) {
                if (host.contains("ip") && host.contains("port") &&
                    host.contains("instanceId")) {
                    ServiceInstance instance;
                    instance.service_name = service_name;
                    instance.instance_id =
                        host.at("instanceId").get<std::string>();
                    instance.address =
                        "tcp://" + host.at("ip").get<std::string>() + ":" +
                        std::to_string(host.at("port").get<int>());
                    // Nacos metadata is usually a map, simplify for now
                    instance.metadata = ServiceMetadata{};
                    // Nacos instances are considered active if returned by list
                    // API
...
std::vector<ServiceInstance> NacosServiceDiscovery::query_all_services(
    const std::string& service_name) {
...
std::vector<ServiceInstance> NacosServiceDiscovery::query_services_by_metadata(
    ...
    if (service_name_it == metadata_filters.end()) {
        SHIELD_LOG_ERROR << "Nacos query_services_by_metadata requires a "
...
void NacosServiceDiscovery::_heartbeat_loop() {
    while (_running_heartbeat) {
        ...
        std::unique_lock<std::mutex> instances_lock(
            _registered_instances_mutex);
        for (const auto& pair : _registered_instances) {
            const ServiceInstance& instance = pair.second;
            std::stringstream ss;
            ss << "serviceName=" << instance.service_name
               << "&instanceId=" << instance.instance_id << "&ephemeral=true";

            std::string target = "/nacos/v1/ns/instance/beat";
            std::string body = ss.str();

            // Nacos beat API expects a PUT request with JSON body for instance
            // details However, the simple beat API is a GET/PUT with query
            // params. Let's use the simple beat API for now. The Nacos
            // documentation is a bit confusing here, but the common client
            // behavior is to send a PUT request to /nacos/v1/ns/instance/beat
            // with instanceId and serviceName. Some clients also send a JSON
            // body with more details. For simplicity, we'll stick to query
            // parameters.
...
std::unique_ptr<IServiceDiscovery> make_nacos_discovery(
    const std::string& nacos_server_address,
    std::chrono::seconds heartbeat_interval) {
    return std::make_unique<NacosServiceDiscovery>(nacos_server_address,
                                                   heartbeat_interval);
}
