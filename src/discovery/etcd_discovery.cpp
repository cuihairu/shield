...
public:
    explicit EtcdDiscoveryImpl(const std::string& endpoints) {
        SHIELD_LOG_INFO << "EtcdServiceDiscovery (STUB) initialized with endpoints: "
                        << endpoints;
    }
    ...
    bool register_service(const ServiceInstance& instance,
        ...
        SHIELD_LOG_WARN << "EtcdServiceDiscovery::register_service is currently stubbed";
        return false;
    }

    bool deregister_service(const std::string& service_name,
                            const std::string& instance_id) {
        SHIELD_LOG_WARN << "EtcdServiceDiscovery::deregister_service is currently stubbed";
        return false;
    }

    std::optional<ServiceInstance> query_service(const std::string& service_name) {
        SHIELD_LOG_WARN << "EtcdServiceDiscovery::query_service is currently stubbed";
        return std::nullopt;
    }

    std::vector<ServiceInstance> query_all_services(const std::string& service_name) {
        SHIELD_LOG_WARN << "EtcdServiceDiscovery::query_all_services is currently stubbed";
        return {};
    }

    std::vector<ServiceInstance> query_services_by_metadata(
        const std::map<std::string, std::string>& metadata_filters) {
        SHIELD_LOG_WARN << "EtcdServiceDiscovery::query_services_by_metadata is currently stubbed";
        return {};
    }
    ...
    std::vector<ServiceInstance> query_services_by_criteria(
        ...
        SHIELD_LOG_WARN << "EtcdServiceDiscovery::query_services_by_criteria is currently stubbed";
        return {};
    }
...
bool EtcdServiceDiscovery::deregister_service(
...
std::vector<ServiceInstance> EtcdServiceDiscovery::query_services_by_criteria(
    ...
    return _impl->query_services_by_criteria(service_name, version_filter, 
                                           region_filter, environment_filter, required_tags);
}
