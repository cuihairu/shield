...
public:
    explicit EtcdDiscoveryImpl(const std::string& endpoints)
        ...
        SHIELD_LOG_INFO << "EtcdServiceDiscovery initialized with endpoints: "
                        << endpoints;
    }
    ...
    bool register_service(const ServiceInstance& instance,
        ...
        long lease_ttl =
            ttl.has_value() ? ttl->count() : 60;  // Default to 60s TTL
        etcd::Response resp = _sync_client.leasegrant(lease_ttl);

        if (!resp.is_ok()) {
            SHIELD_LOG_ERROR << "Failed to grant etcd lease: "
                             << resp.error_message();
            return false;
        }
        ...
        if (!resp.is_ok()) {
            SHIELD_LOG_ERROR << "Failed to put service key to etcd: "
                             << resp.error_message();
        ...
        try {
            ...
            SHIELD_LOG_ERROR << "Failed to start lease keep-alive: "
                             << e.what();
            _sync_client.leaserevoke(lease_id);
            return false;
        }
    ...
    bool deregister_service(const std::string& service_name,
        ...
        if (keepalive_to_cancel) {
            ...
            SHIELD_LOG_INFO << "Deregistered service with ID '" << instance_id
                            << "' by revoking lease.";
            return true;
        } else {
            ...
            if (resp.is_ok()) {
                SHIELD_LOG_INFO << "Deregistered service with ID '"
                                << instance_id << "' by deleting key.";
                return true;
            }
            SHIELD_LOG_WARN
                << "Could not find lease for instance ID '" << instance_id
                << "' and failed to delete key: " << resp.error_message();
            return false;
        }
    }
    ...
    std::vector<ServiceInstance> query_all_services(
        ...
        if (!resp.is_ok()) {
            SHIELD_LOG_ERROR << "Failed to query etcd with prefix " << prefix
                             << ": " << resp.error_message();
            return instances;
        }
        ...
        for (const auto& val : resp.values()) {
            try {
                ServiceInstance instance =
                    nlohmann::json::parse(val.as_string());
                instances.push_back(instance);
            } catch (const std::exception& e) {
                SHIELD_LOG_ERROR
                    << "Failed to parse service instance JSON for key "
                    << val.key() << ": " << e.what();
            }
        }
        return instances;
...
std::vector<ServiceInstance> EtcdServiceDiscovery::query_services_by_metadata(
    ...
    if (service_name_it == metadata_filters.end()) {
        SHIELD_LOG_ERROR << "etcd query_services_by_metadata requires a "
