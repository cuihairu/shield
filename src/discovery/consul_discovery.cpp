...
ConsulServiceDiscovery::ConsulServiceDiscovery(const std::string& consul_server_address, std::chrono::seconds check_interval)
...
std::string ConsulServiceDiscovery::_send_http_request(http::verb method, const std::string& target, const std::string& body, const std::string& content_type) const {
    net::io_context ioc;
...
bool ConsulServiceDiscovery::register_service(const ServiceInstance& instance, std::optional<std::chrono::seconds> ttl) {
    ...
    std::string ip = instance.address.substr(instance.address.find("//") + 2, instance.address.find(":", instance.address.find("//") + 2) - (instance.address.find("//") + 2));
    int port = std::stoi(instance.address.substr(instance.address.find(":", instance.address.find("//") + 2) + 1));
    service_json["Address"] = ip;
    service_json["Port"] = port;
    // Convert metadata to Consul tags
    nlohmann::json meta_json = nlohmann::json::parse(instance.metadata.to_json());
    ...
    check_json["DeregisterCriticalServiceAfter"] = "1m"; // Deregister after 1 minute if check fails
    check_json["TTL"] = std::to_string(ttl ? ttl->count() : 10) + "s"; // Default TTL to 10s if not provided
    check_json["CheckID"] = check_id;
    check_json["Name"] = "service:" + instance.service_name + ":" + instance.instance_id + ":healthcheck";
...
bool ConsulServiceDiscovery::deregister_service(const std::string& service_name, const std::string& instance_id) {
...
std::optional<ServiceInstance> ConsulServiceDiscovery::query_service(const std::string& service_name) {
    ...
    try {
        ...
        if (j.is_array()) {
            ...
            for (const auto& entry : j) {
                if (entry.contains("ServiceAddress") && entry.contains("ServicePort") && entry.contains("ServiceID")) {
                    ServiceInstance instance;
                    instance.service_name = service_name;
                    instance.instance_id = entry.at("ServiceID").get<std::string>();
                    instance.address = "tcp://" + entry.at("ServiceAddress").get<std::string>() + ":" + std::to_string(entry.at("ServicePort").get<int>());
                    instance.metadata = entry.contains("ServiceMeta") ? ServiceMetadata::from_json(entry.at("ServiceMeta").dump()) : ServiceMetadata{};
                    instance.expiration_time = std::chrono::steady_clock::time_point::max(); // Consul manages health, not explicit TTL here
        ...
        std::cerr << "Consul query_service JSON parse error: " << e.what() << std::endl;
    } catch (const nlohmann::json::exception& e) {
        std::cerr << "Consul query_service JSON exception: " << e.what() << std::endl;
    }

    return std::nullopt;
}

std::vector<ServiceInstance> ConsulServiceDiscovery::query_all_services(const std::string& service_name) {
    ...
    try {
        ...
        std::cerr << "Consul query_all_services JSON parse error: " << e.what() << std::endl;
    } catch (const nlohmann::json::exception& e) {
        std::cerr << "Consul query_all_services JSON exception: " << e.what() << std::endl;
    }

    return instances;
}

std::vector<ServiceInstance> ConsulServiceDiscovery::query_services_by_metadata(const std::map<std::string, std::string>& metadata_filters) {
    std::vector<ServiceInstance> matching_instances;
    if (metadata_filters.empty()) {
        return matching_instances;
    }

    // For Consul, we can only filter by tags. We assume the service name is one of the filters.
    auto service_name_it = metadata_filters.find("service_name");
    if (service_name_it == metadata_filters.end()) {
        // Or, we could query all services and filter client-side, but that's inefficient.
        // For now, we require a service_name for a targeted query.
        std::cerr << "Consul query_services_by_metadata requires a 'service_name' in filters." << std::endl;
        return matching_instances;
    }
    ...
    try {
        ...
        std::cerr << "Consul query_services_by_metadata JSON parse error: " << e.what() << std::endl;
    } catch (const nlohmann::json::exception& e) {
        std::cerr << "Consul query_services_by_metadata JSON exception: " << e.what() << std::endl;
    }
...
void ConsulServiceDiscovery::_check_loop() {
    while (_running_check) {
        ...
        _check_cv.wait_for(lock, _check_interval, [this] { return !_running_check; });
        ...
        for (const auto& pair : _registered_checks) {
            ...
            std::string response = _send_http_request(http::verb::put, target, "", "text/plain"); // Content-Type can be anything, or omitted
            if (!response.empty()) {
                std::cerr << "Consul heartbeat failed for check " << check_id << ": " << response << std::endl;
            }
        }
    }
}

std::unique_ptr<IServiceDiscovery> make_consul_discovery(const std::string& consul_server_address, std::chrono::seconds check_interval) {
    return std::make_unique<ConsulServiceDiscovery>(consul_server_address, check_interval);
}
