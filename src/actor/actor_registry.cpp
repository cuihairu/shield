...
std::string ActorMetadata::type_to_string() const {
    switch (type) {
        case ActorType::GATEWAY: return "gateway";
        case ActorType::LOGIC: return "logic";
        case ActorType::DATABASE: return "database";
        case ActorType::AUTH: return "auth";
        case ActorType::MONITOR: return "monitor";
...
RegisteredActor::RegisteredActor(const caf::actor& actor, const ActorMetadata& meta, 
                                const std::string& uri, bool local)
    : actor_handle(actor), metadata(meta), actor_uri(uri), is_local(local) {
}

ActorRegistry::ActorRegistry(caf::actor_system& system, 
                           std::shared_ptr<discovery::IServiceDiscovery> discovery_service,
                           const std::string& node_id)
    : actor_system_(system), discovery_service_(discovery_service), node_id_(node_id),
...
ActorRegistry::~ActorRegistry() {
    ...
    for (const auto& [name, registered_actor] : local_actors_) {
        discovery_service_->deregister_service("actor:" + registered_actor.metadata.type_to_string(), 
...
bool ActorRegistry::register_actor(const caf::actor& actor, 
    ...
    if (actor_uri.empty()) {
        SHIELD_LOG_ERROR << "Failed to generate URI for actor: " << metadata.name;
        return false;
    }
    ...
    if (!discovery_service_->register_service(service_instance, ttl)) {
        SHIELD_LOG_ERROR << "Failed to register actor with discovery service: " << metadata.name;
        return false;
    }
    ...
    SHIELD_LOG_INFO << "Actor registered successfully: " << metadata.name 
                   << " (type: " << metadata.type_to_string() << ", uri: " << actor_uri << ")";
    return true;
}
...
bool ActorRegistry::unregister_actor(const std::string& actor_name) {
    ...
    if (it == local_actors_.end()) {
        SHIELD_LOG_WARNING << "Actor not found for unregistration: " << actor_name;
        return false;
    }

    const auto& registered_actor = it->second;
    std::string service_name = "actor:" + registered_actor.metadata.type_to_string();
    
    // Deregister from discovery service
    if (!discovery_service_->deregister_service(service_name, actor_name)) {
        SHIELD_LOG_WARNING << "Failed to deregister actor from discovery service: " << actor_name;
    }
...
caf::actor ActorRegistry::find_local_actor(const std::string& actor_name) const {
    std::lock_guard<std::mutex> lock(registry_mutex_);
...
std::vector<RegisteredActor> ActorRegistry::find_local_actors_by_type(ActorType type) const {
    ...
    for (const auto& service_type : service_types) {
        ...
        for (const auto& instance : instances) {
            if (instance.instance_id == actor_name) {
                ...
                if (uri_result) {
                    auto& uri = *uri_result;
                    auto remote_actor = actor_system_.middleman().remote_actor(uri.authority().host_str(), uri.authority().port);
                    if (remote_actor) {
                        // Cache the remote actor
                        RegisteredActor registered_actor = service_instance_to_registered_actor(instance);
                        registered_actor.actor_handle = caf::actor_cast<caf::actor>(*remote_actor);
                        ...
                        SHIELD_LOG_INFO << "Discovered remote actor: " << actor_name 
                                       << " at " << instance.address;
                        return *remote_actor;
                    }
                }
...
std::vector<RegisteredActor> ActorRegistry::discover_remote_actors_by_type(ActorType type) {
    std::vector<RegisteredActor> result;
    std::string service_name = "actor:" + ActorMetadata{type, "", "", "", {}, 100, {}}.type_to_string();
    ...
    for (const auto& instance : instances) {
        ...
        if (uri_result) {
            ...
            if (remote_actor) {
                ...
                // Cache the remote actor
                std::lock_guard<std::mutex> lock(registry_mutex_);
                remote_actors_cache_[registered_actor.metadata.name] = registered_actor;
            }
        }
    }
    
    SHIELD_LOG_DEBUG << "Discovered " << result.size() << " remote actors of type: " 
                    << ActorMetadata{type, "", "", "", {}, 100, {}}.type_to_string();
    return result;
}

std::vector<RegisteredActor> ActorRegistry::discover_actors_by_group(const std::string& service_group) {
    std::vector<RegisteredActor> result;
    ...
    for (const auto& service_type : service_types) {
        ...
        for (const auto& instance : instances) {
            if (instance.metadata.custom_attributes.count("service_group") &&
                instance.metadata.custom_attributes.at("service_group") == service_group) {
                ...
                if (instance.metadata.custom_attributes.count("node_id") &&
                    ...
                    // Local actor
                    registered_actor.actor_handle = find_local_actor(registered_actor.metadata.name);
                }
...
void ActorRegistry::set_discovery_callback(const ActorDiscoveryCallback& callback) {
...
void ActorRegistry::start_heartbeat(std::chrono::seconds interval) {
    ...
    SHIELD_LOG_INFO << "Actor registry heartbeat started with interval: " << interval.count() << "s";
}
...
discovery::ServiceInstance ActorRegistry::metadata_to_service_instance(
    ...
    instance.metadata.custom_attributes["service_group"] = metadata.service_group;
    instance.metadata.custom_attributes["actor_type"] = metadata.type_to_string();
...
RegisteredActor ActorRegistry::service_instance_to_registered_actor(
    ...
    registered_actor.metadata.node_id = instance.metadata.custom_attributes.count("node_id") ?
                                       instance.metadata.custom_attributes.at("node_id") : "";
    registered_actor.metadata.service_group = instance.metadata.custom_attributes.count("service_group") ?
...
std::string ActorRegistry::generate_actor_uri(const caf::actor& actor) const {
    try {
        ...
        auto port = actor_system_.middleman().publish(actor, 0);  // 0 = any available port
        if (!port) {
            SHIELD_LOG_ERROR << "Failed to publish actor: " << to_string(port.error());
            return "";
        }
        ...
        SHIELD_LOG_ERROR << "Exception while generating actor URI: " << e.what();
        return "";
    }
}
...
void ActorRegistry::heartbeat_worker() {
    while (heartbeat_running_) {
        try {
            ...
            SHIELD_LOG_DEBUG << "Heartbeat sent for " << actors_to_update.size() << " actors";
...
void ActorRegistry::update_heartbeat(const std::string& actor_name) {
    ...
    if (it != local_actors_.end()) {
        ...
        auto service_instance = metadata_to_service_instance(it->second.metadata, it->second.actor_uri);
        std::string service_name = "actor:" + it->second.metadata.type_to_string();
        
        discovery_service_->register_service(service_instance, heartbeat_interval_ * 2);  // TTL = 2x heartbeat interval
    }
}
