...
ActorSystemCoordinator::ActorSystemCoordinator(const CoordinatorConfig& config)
    ...
    SHIELD_LOG_INFO << "ActorSystemCoordinator created for node: " << config_.node_id;
}
...
bool ActorSystemCoordinator::initialize() {
    ...
    try {
        ...
        emit_status("CAF actor system created with " + std::to_string(config_.worker_threads) + " workers");
        ...
        distributed_system_ = make_distributed_actor_system(*actor_system_, discovery_service_, dist_config);
        ...
        distributed_system_->set_event_callback(
            [this](const ActorSystemEventData& event_data) { handle_system_event(event_data); });
...
bool ActorSystemCoordinator::start() {
    ...
    try {
        ...
        running_ = true;
        emit_status("ActorSystemCoordinator started successfully on node: " + config_.node_id);
        ...
        std::string error_msg = "Exception during start: " + std::string(e.what());
        emit_status(error_msg, true);
        SHIELD_LOG_ERROR << error_msg;
        running_ = false;
        return false;
...
void ActorSystemCoordinator::stop() {
    ...
    try {
        ...
        std::string error_msg = "Exception during stop: " + std::string(e.what());
        emit_status(error_msg, true);
        SHIELD_LOG_ERROR << error_msg;
    }
}

bool ActorSystemCoordinator::register_actor(const caf::actor& actor,
                                           ActorType type,
    ...
    bool success = distributed_system_->register_actor(actor, type, name, service_group, tags);
    if (success) {
        ++total_actors_registered_;
        emit_status("Actor registered: " + name + " (type: " + 
...
std::vector<RegisteredActor> ActorSystemCoordinator::find_actors_by_type(ActorType type) {
    if (!running_) {
        return {};
    }
...
std::map<std::string, std::string> ActorSystemCoordinator::get_cluster_status() {
    ...
    status["total_actors_registered"] = std::to_string(total_actors_registered_.load());
    ...
    if (running_ && distributed_system_) {
        ...
        status["cluster_total_nodes"] = std::to_string(cluster_stats.total_nodes);
        status["cluster_total_actors"] = std::to_string(cluster_stats.total_actors);
        status["cluster_local_actors"] = std::to_string(cluster_stats.local_actors);
        status["cluster_remote_actors"] = std::to_string(cluster_stats.remote_actors);
        status["healthy"] = distributed_system_->is_healthy() ? "true" : "false";
    } else {
...
void ActorSystemCoordinator::set_status_callback(const StatusCallback& callback) {
    status_callback_ = callback;
}

std::shared_ptr<discovery::IServiceDiscovery> ActorSystemCoordinator::create_discovery_service() {
    if (config_.discovery_type == "etcd") {
        if (config_.discovery_endpoints.empty()) {
            SHIELD_LOG_ERROR << "ETCD endpoints not configured";
            return nullptr;
        }
        return shield::discovery::make_etcd_discovery(config_.discovery_endpoints);
    } else if (config_.discovery_type == "in-memory") {
        return shield::discovery::make_local_discovery();
    } else {
        SHIELD_LOG_ERROR << "Unknown discovery service type: " << config_.discovery_type;
        return nullptr;
    }
}

std::unique_ptr<caf::actor_system> ActorSystemCoordinator::create_actor_system() {
...
void ActorSystemCoordinator::handle_system_event(const ActorSystemEventData& event_data) {
    std::ostringstream oss;
    ...
    switch (event_data.event_type) {
        ...
        case ActorSystemEvent::ACTOR_DISCOVERED:
            oss << "Actor discovered: " << event_data.actor_name 
                << " on node " << event_data.node_id;
            break;
        case ActorSystemEvent::ACTOR_LOST:
            oss << "Actor lost: " << event_data.actor_name 
                << " on node " << event_data.node_id;
            break;
...
void ActorSystemCoordinator::emit_status(const std::string& status, bool is_error) {
    if (is_error) {
...
std::unique_ptr<ActorSystemCoordinator> make_coordinator_from_config(
    ...
    // Note: This assumes the Shield config has been extended with actor system settings
...
std::unique_ptr<ActorSystemCoordinator> make_default_coordinator(const std::string& node_id) {
    CoordinatorConfig config;
