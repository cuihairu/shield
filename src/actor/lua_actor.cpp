...
LuaActor::LuaActor(caf::actor_config& cfg, 
    ...
    , actor_id_(actor_id.empty() ? "lua_actor_" + std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count()) : actor_id)
    , script_loaded_(false) {
    
    // Acquire Lua VM from pool
    m_lua_vm_handle = m_lua_vm_pool.acquire_vm();
    if (!m_lua_vm_handle) {
        SHIELD_LOG_ERROR << "Failed to acquire Lua VM for LuaActor: " << actor_id_;
    ...
    SHIELD_LOG_INFO << "LuaActor created with ID: " << actor_id_ << ", script: " << script_path_;
}

caf::behavior LuaActor::make_behavior() {
    // Load the Lua script
    if (!load_script()) {
        SHIELD_LOG_ERROR << "Failed to load script for LuaActor: " << script_path_;
        return {
            [this](const std::string& msg_type, const std::string& data_json) -> std::string {
...
void LuaActor::register_cpp_functions() {
    ...
    m_lua_vm_handle->register_function("log_info", [this](const std::string& msg) {
        lua_log_info(msg);
    });
    
    m_lua_vm_handle->register_function("log_error", [this](const std::string& msg) {
        lua_log_error(msg);
    });
    
    // Register message sending function
    m_lua_vm_handle->register_function("send_message", 
    ...
    m_lua_vm_handle->register_function("get_actor_id", [this]() -> std::string {
        return actor_id_;
    });
}
...
LuaResponse LuaActor::handle_lua_message(const LuaMessage& msg) {
    ...
    try {
        ...
        sol::protected_function on_message_func = m_lua_vm_handle->lua()["on_message"];
        if (!on_message_func.valid()) {
            SHIELD_LOG_ERROR << "Lua script does not have an 'on_message' function.";
            return LuaResponse(false, {}, "Lua script does not have 'on_message' function");
        }
        ...
        if (result.valid()) {
            ...
            std::string error_message = error_message_obj.valid() ? error_message_obj.as<std::string>() : "";
            std::unordered_map<std::string, std::string> response_data;
            sol::object data_table_obj = response_table["data"];
            sol::table data_table = data_table_obj.valid() ? data_table_obj.as<sol::table>() : m_lua_vm_handle->lua().create_table();
            for (auto pair : data_table) {
                response_data[pair.first.as<std::string>()] = pair.second.as<std::string>();
            }
            return LuaResponse(success, response_data, error_message);
        } else {
            sol::error err = result;
            SHIELD_LOG_ERROR << "Error calling Lua on_message function: " << err.what();
            return LuaResponse(false, {}, std::string("Lua error: ") + err.what());
        }
...
std::string LuaActor::handle_lua_message_json(const std::string& msg_type, const std::string& data_json) {
    if (!script_loaded_) {
        return R"({"success": false, "error_message": "Script not loaded"})";
    }
    ...
    try {
        ...
        if (result.valid()) {
            ...
            response_json["error_message"] = response_table["error_message"].get_or(std::string());
            ...
            error_response["error_message"] = std::string("Lua error: ") + err.what();
        ...
        nlohmann::json error_response;
        error_response["success"] = false;
        error_response["error_message"] = std::string("C++ exception: ") + e.what();
        error_response["data"] = nlohmann::json::object();
        return error_response.dump();
    }
}
...
void LuaActor::lua_send_message(const std::string& target_actor, const std::string& msg_type,
                               const std::unordered_map<std::string, std::string>& data) {
    // For now, just log the message sending attempt
    // In a full implementation, this would send to the distributed actor system
    SHIELD_LOG_INFO << "Lua actor " << actor_id_ << " attempting to send message '" 
                   << msg_type << "' to actor: " << target_actor;
...
caf::actor create_lua_actor(caf::actor_system& system, 
    ...
    return system.spawn<LuaActor>(lua_vm_pool, actor_system, script_path, actor_id);
}
