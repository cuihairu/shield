...
bool DistributedActorSystem::initialize() {
    ...
    try {
        ...
        SHIELD_LOG_ERROR << "Failed to initialize DistributedActorSystem: "
                         << e.what();
        return false;
    }
}
...
void DistributedActorSystem::shutdown() {
    ...
    try {
        ...
        SHIELD_LOG_ERROR << "Error during DistributedActorSystem shutdown: "
                         << e.what();
    }
}

bool DistributedActorSystem::register_actor(
    ...
    bool success = actor_registry_->register_actor(
        actor, metadata, config_.heartbeat_interval * 2);
...
std::vector<RegisteredActor> DistributedActorSystem::find_actors_by_type(
    ...
    if (include_remote) {
        auto remote_actors =
            actor_registry_->discover_remote_actors_by_type(type);
...
DistributedActorSystem::get_cluster_topology() {
    ...
    std::vector<std::string> actor_type_names = {
        "gateway", "logic", "database", "auth", "monitor", "custom"};
...
DistributedActorSystem::ClusterStats
DistributedActorSystem::get_cluster_stats() {
    ClusterStats stats{};
    ...
    std::vector<ActorType> types = {ActorType::GATEWAY,  ActorType::LOGIC,
                                    ActorType::DATABASE, ActorType::AUTH,
                                    ActorType::MONITOR,  ActorType::CUSTOM};
...
void DistributedActorSystem::discovery_worker() {
    while (discovery_running_) {
        try {
            ...
            SHIELD_LOG_DEBUG << "Discovery refresh completed. Known nodes: "
                             << current_nodes.size();
...
void DistributedActorSystem::emit_event(
...
std::unique_ptr<DistributedActorSystem> make_distributed_actor_system(
    ...
    std::shared_ptr<discovery::IServiceDiscovery> discovery_service,
    const DistributedActorConfig& config) {
    return std::make_unique<DistributedActorSystem>(actor_system,
