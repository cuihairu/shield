...
void ActorStarter::initialize(core::ApplicationContext& context) {
    ...
    auto discovery_config =
        config_manager.get_configuration_properties<discovery::DiscoveryConfig>();

    size_t worker_threads = 0;
    std::string node_id = "shield_node_" + std::to_string(std::random_device{}());
    std::string cluster_name = "shield_cluster";
    uint16_t actor_port = 0;

    if (actor_system_config) {
        worker_threads =
            static_cast<size_t>(actor_system_config->get_effective_worker_threads());
    ...
    auto caf_system =
        context.register_bean<caf::actor_system>("caf_actor_system", caf_config);
    ...
    if (discovery_config) {
        if (discovery_config->type == "redis") {
            ...
            for (size_t i = 0; i < discovery_config->etcd.endpoints.size(); ++i) {
                if (i > 0) endpoints += ",";
