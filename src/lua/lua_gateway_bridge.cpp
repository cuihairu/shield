...
void LuaGatewayBridge::on_connect(std::shared_ptr<shield::net::Session> session) {
    ...
    manager_.enqueue_forked_task(target, [manager, target, session_info]() {
        ...
        if (!manager->send_system(target, "on_connect",
            ...
            SHIELD_LOG_WARNING(log, "Failed to queue gateway on_connect: " + error);
        }
    });
}
...
void LuaGatewayBridge::on_message(std::shared_ptr<shield::net::Session> session,
    ...
    auto* manager = &manager_;
    const auto target = gateway_service_name_;
    manager_.enqueue_forked_task(target, [manager, target, session_info, payload]() {
        std::string error;
        if (!manager->send_system(target, "on_client_message",
            ...
            SHIELD_LOG_WARNING(log,
                               "Failed to queue gateway on_client_message: " + error);
        }
    });
}
...
void LuaGatewayBridge::on_disconnect(std::shared_ptr<shield::net::Session> session,
    ...
    manager_.enqueue_forked_task(
        target, [manager, target, session_info, reason = std::move(reason)]() {
            std::string error;
            if (!manager->send_system(target, "on_disconnect",
                                      nlohmann::json::array({session_info, reason}),
                                      &error)) {
                auto& log = shield::log::get_logger("lua");
                SHIELD_LOG_WARNING(log,
                                   "Failed to queue gateway on_disconnect: " + error);
            }
        });
}
