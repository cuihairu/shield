...
PooledLuaVM::PooledLuaVM(const std::string& vm_id)
    : vm_id_(vm_id), last_used_(std::chrono::steady_clock::now()) {
...
bool PooledLuaVM::initialize() {
    try {
        ...
        SHIELD_LOG_ERROR << "Failed to initialize PooledLuaVM " << vm_id_
                         << ": " << e.what();
        healthy_ = false;
        return false;
    }
...
void PooledLuaVM::reset() {
    try {
        ...
        SHIELD_LOG_ERROR << "Failed to reset PooledLuaVM " << vm_id_ << ": "
                         << e.what();
        healthy_ = false;
    }
}
...
void LuaVMPool::on_start() {
    ...
    SHIELD_LOG_INFO << "LuaVMPool started with " << available_vms_.size()
                    << " VMs";
}
...
void LuaVMPool::preload_scripts_to_vm(PooledLuaVM* vm) {
    ...
    for (const auto& script_path : script_paths_) {
        if (!vm->engine()->load_script(script_path)) {
            SHIELD_LOG_WARN << "Failed to preload script " << script_path
                            << " to VM " << vm->vm_id();
        }
    }
    ...
    for (const auto& [name, content] : preloaded_scripts_) {
        if (!vm->engine()->execute_string(content)) {
            SHIELD_LOG_WARN << "Failed to preload script content '" << name
                            << "' to VM " << vm->vm_id();
        }
    }
}
...
VMHandle LuaVMPool::acquire_vm(std::chrono::milliseconds timeout) {
    ...
    bool success = pool_condition_.wait_for(
        lock, timeout, [this] { return !available_vms_.empty() || !running_; });
    ...
    if (!success || available_vms_.empty()) {
        ...
        if (all_vms_.size() < config_.max_size) {
            ...
            if (vm && initialize_vm(vm.get())) {
                all_vms_[vm->vm_id()] = vm;
                available_vms_.push(vm);
                SHIELD_LOG_DEBUG << "Expanded pool to " << all_vms_.size()
                                 << " VMs";
            }
        }
        ...
        if (available_vms_.empty()) {
            failed_acquisitions_++;
            SHIELD_LOG_WARN
                << "Failed to acquire VM from pool - timeout or pool exhausted";
            return {};
        }
    }
    ...
    if (!vm->is_healthy()) {
        ...
        vm->reset();
        if (!vm->is_healthy()) {
            SHIELD_LOG_ERROR << "VM " << vm->vm_id()
                             << " is unhealthy and cannot be reset";
            failed_acquisitions_++;
            return {};
        }
    }
    ...
    total_wait_time_ms_ =
        (total_wait_time_ms_ * (total_acquisitions_ - 1) + wait_time) /
        total_acquisitions_;

    // Create RAII handle
    return VMHandle(vm, [this, vm]() { return_vm(vm); });
}
...
void LuaVMPool::return_vm(std::shared_ptr<PooledLuaVM> vm) {
    ...
    if (vm->is_healthy() && running_) {
        ...
        all_vms_.erase(it);
        SHIELD_LOG_INFO << "Removed unhealthy VM " << vm->vm_id()
                        << " from pool";
    }
}
...
bool LuaVMPool::preload_script_content(const std::string& script_name,
...
void LuaVMPool::resize_pool(size_t new_size) {
    ...
    if (new_size > current_size) {
        ...
        SHIELD_LOG_INFO << "Expanded pool from " << current_size << " to "
                        << all_vms_.size() << " VMs";
    } else if (new_size < current_size) {
        // Shrink pool (remove only available VMs)
        size_t to_remove =
            std::min(current_size - new_size, available_vms_.size());
        ...
        SHIELD_LOG_INFO << "Shrunk pool from " << current_size << " to "
                        << all_vms_.size() << " VMs";
    }
}
...
void LuaVMPool::cleanup_idle_vms() {
    ...
    while (!available_vms_.empty()) {
        ...
        if (idle_time > config_.idle_timeout &&
            all_vms_.size() > config_.min_size) {
            // Remove idle VM
            all_vms_.erase(vm->vm_id());
            removed_count++;
        } else {
    ...
    if (removed_count > 0) {
        SHIELD_LOG_INFO << "Cleaned up " << removed_count
                        << " idle VMs, pool size: " << all_vms_.size();
    }
}
