...
CLICommand::CLICommand() 
    : Command("cli", "Command line interface for Shield server management") {
    setup_flags();
    set_long_description("Connect to a running Shield server and execute management commands.")
        .set_usage("shield cli [OPTIONS] [COMMAND]")
        .set_example("  shield cli --url http://localhost:8080\\n"
...
int CLICommand::run(shield::core::CommandContext& ctx) {
    // Load CLI-specific layered configuration
    auto& config = shield::core::Config::instance();
    try {
        config.load_for_cli();
    } catch (const std::exception& e) {
        std::cerr << "Warning: Failed to load CLI configuration: " << e.what() << std::endl;
