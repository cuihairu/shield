...
int main(int argc, char* argv[]) {
    ...
    try {
        config.load(options.config_file.empty() ? "config/shield.yaml" : options.config_file);
    ...
    log_config.level = shield::core::Logger::level_from_string(config.get<std::string>("logger.level"));
    ...
    std::shared_ptr<shield::discovery::IServiceDiscovery> discovery_service = std::move(discovery_service_unique);

    // Distributed Actor System
    shield::actor::DistributedActorConfig actor_config;
    
    // Configure dynamic node ID
    std::string node_id_config = config.get<std::string>("actor_system.node_id");
    if (node_id_config == "auto") {
        ...
        actor_config.node_id = hostname + "_" + std::to_string(process_id) + "_" + std::to_string(timestamp);
        SHIELD_LOG_INFO << "Generated dynamic node ID: " << actor_config.node_id;
    } else {
    ...
    shield::actor::DistributedActorSystem distributed_actor_system(system, discovery_service, actor_config);
        ...
        if (name == "prometheus") {
            ...
            for (auto& comp : components) {
                if (comp->name() == "lua_vm_pool") {
                    lua_vm_pool_ptr = static_cast<shield::script::LuaVMPool*>(comp.get());
                    break;
                }
            }
            if (!lua_vm_pool_ptr) {
                SHIELD_LOG_ERROR << "LuaVMPool component not found, cannot initialize GatewayComponent.";
                return 1; // Exit if LuaVMPool is not available
            }
            components.emplace_back(std::make_unique<shield::gateway::GatewayComponent>("gateway", distributed_actor_system, *lua_vm_pool_ptr));
        } else if (name == "lua_vm_pool") {
            shield::script::LuaVMPoolConfig lua_vm_pool_config;
            
            // Load LuaVMPoolConfig from config file
            try {
                lua_vm_pool_config.initial_size = config.get<size_t>("lua_vm_pool.initial_size");
                lua_vm_pool_config.min_size = config.get<size_t>("lua_vm_pool.min_size");
                lua_vm_pool_config.max_size = config.get<size_t>("lua_vm_pool.max_size");
                lua_vm_pool_config.idle_timeout = std::chrono::milliseconds(config.get<int>("lua_vm_pool.idle_timeout_ms"));
                lua_vm_pool_config.acquire_timeout = std::chrono::milliseconds(config.get<int>("lua_vm_pool.acquire_timeout_ms"));
                lua_vm_pool_config.preload_scripts = config.get<bool>("lua_vm_pool.preload_scripts");
                
                SHIELD_LOG_INFO << "LuaVMPool configured: initial=" << lua_vm_pool_config.initial_size 
                               << ", min=" << lua_vm_pool_config.min_size 
                               << ", max=" << lua_vm_pool_config.max_size
                               << ", preload_scripts=" << lua_vm_pool_config.preload_scripts;
            } catch (const std::exception& e) {
                SHIELD_LOG_WARN << "Failed to load LuaVMPool configuration, using defaults: " << e.what();
                // Keep default values from LuaVMPoolConfig constructor
            }
            
            components.emplace_back(std::make_unique<shield::script::LuaVMPool>("lua_vm_pool", lua_vm_pool_config));
        } else if (name == "logger") {
