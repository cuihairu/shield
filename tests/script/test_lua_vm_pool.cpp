...
void test_vm_pool_concurrency() {
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([&pool, &successful_operations, &failed_operations, operations_per_thread, t]() {
            for (int i = 0; i < operations_per_thread; ++i) {
                auto vm_handle = pool.acquire_vm(std::chrono::milliseconds{2000});
                if (vm_handle) {
                    // Simulate some work
                    std::string script = "thread_" + std::to_string(t) + "_var = " + std::to_string(i);
    ...
    std::cout << "Concurrent test results - Successful: " << successful_operations.load()
              << ", Failed: " << failed_operations.load() << std::endl;
    
    assert(successful_operations.load() > 0 && "No successful concurrent operations");
    assert(failed_operations.load() < successful_operations.load() * 0.1 && "Too many failed operations");
    
    auto final_stats = pool.get_stats();
    std::cout << "Final pool stats - Total VMs: " << final_stats.total_vms
              << ", Total acquisitions: " << final_stats.total_acquisitions << std::endl;
...
void test_vm_pool_script_preloading() {
    ...
    assert(preloaded_var && *preloaded_var == "Hello from preloaded script!" && "Preloaded variable not found");
...
void test_vm_pool_cleanup() {
    ...
    config.max_size = 6;
    config.min_size = 2;
    config.idle_timeout = std::chrono::milliseconds{500}; // Short timeout for testing
    ...
    assert(stats_after.total_vms >= config.min_size && "Pool shrunk below minimum");
    assert(stats_after.total_vms <= stats_before.total_vms && "Pool didn't shrink");
...
int main() {
    try {
        ...
        std::cout << "\n🎉 All LuaVMPool tests passed! VM pool management is working!" << std::endl;
        return 0;
