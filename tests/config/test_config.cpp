...
struct ConfigFixture {
    ...
    fs::path create_temp_yaml(const std::string& filename, const std::string& content) {
...
database:
  ...
  path: /var/data/db.sqlite
    ...
    fs::path config_path = fixture.create_temp_yaml("nested.yaml", yaml_content);
...
{
    shield::core::Config& config = shield::core::Config::instance();
    // Assume load method throws exception or returns false when file doesn't exist
    BOOST_CHECK_THROW(config.load("non_existent_file.yaml"), std::runtime_error); // or other exception type you defined
}
