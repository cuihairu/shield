...
TEST_F(DistributedActorSystemTest, ActorDiscovery) {
    ...
    // Find actors by type
    auto logic_actors = distributed_system_->find_actors_by_type(ActorType::LOGIC);
    EXPECT_EQ(logic_actors.size(), 1);
    EXPECT_EQ(logic_actors[0].metadata.name, "logic_1");

    auto gateway_actors = distributed_system_->find_actors_by_type(ActorType::GATEWAY);
