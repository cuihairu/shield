...
public:
    ...
    bool load_script() {
        ...
        try {
            ...
            if (success) {
                ...
                if (!init_result) {
                    SHIELD_LOG_INFO << "No on_init function found in script "
                                       "(this is optional)";
                }
            }
            return success;
    ...
    LuaResponse process_message(const LuaMessage &msg) {
        ...
        try {
            ...
            std::string lua_code =
                "current_message = {" + std::string("type = \"") + msg.type +
            ...
            bool call_success = lua_engine_->execute_string(
                "response = on_message(current_message)");
            ...
            if (call_success) {
                ...
                // Get simple values
                auto success =
                    lua_engine_->get_global<bool>("response_success");
                auto error_msg =
                    lua_engine_->get_global<std::string>("response_error");
                ...
                return LuaResponse(false, {},
                                   "Failed to call Lua on_message function");
            }
...
private:
    ...
    void register_cpp_functions() {
        ...
        lua_engine_->register_function(
            "log_info", [this](const std::string &msg) {
                SHIELD_LOG_INFO << "[" << actor_id_ << "] " << msg;
            });

        lua_engine_->register_function(
            "log_error", [this](const std::string &msg) {
