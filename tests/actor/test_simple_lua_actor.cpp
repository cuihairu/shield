...
public:
  ...
  SimpleLuaActor(const std::string &script_path)
    ...
    SHIELD_LOG_INFO << "SimpleLuaActor created with ID: " << actor_id_
                    << ", script: " << script_path_;
  }
  ...
  LuaResponse process_message(const LuaMessage &msg) {
    ...
    try {
      ...
      std::string lua_code = "current_message = {" + std::string("type = \"") +
                             msg.type + "\", " + std::string("sender_id = \"") +
                             msg.sender_id + "\", " + std::string("data = {}}");

      lua_engine_->execute_string(lua_code);

      // Set message data fields
      for (const auto &[key, value] : msg.data) {
        lua_engine_->execute_string("current_message.data[\"" + key +
                                    "\"] = \"" + value + "\"");
      }

      // Call Lua message handler using execute_string
      bool call_success =
          lua_engine_->execute_string("response = on_message(current_message)");
      ...
      if (call_success) {
        ...
        if (success && *success) {
          ...
          if (msg.type == "login") {
            auto player_id =
                lua_engine_->execute_string("return response.data.player_id")
                    ? lua_engine_->get_global<std::string>("player_id")
                    : std::nullopt;
            auto player_name =
                lua_engine_->execute_string("return response.data.player_name")
                    ? lua_engine_->get_global<std::string>("player_name")
                    : std::nullopt;
            auto level =
                lua_engine_->execute_string("return response.data.level")
                    ? lua_engine_->get_global<std::string>("level")
                    : std::nullopt;
            auto health =
                lua_engine_->execute_string("return response.data.health")
        ...
        return LuaResponse(success ? *success : false, response_data,
                           error_msg ? *error_msg : "");
      } else {
...
void test_simple_lua_actor() {
  ...
  SimpleLuaActor::LuaMessage login_msg(
      "login",
      {{"player_name", "TestPlayer"}, {"level", "5"}, {"health", "80"}});

  auto login_response = actor.process_message(login_msg);
  std::cout << "Login response success: " << login_response.success
            << std::endl;
  std::cout << "Login response error: " << login_response.error_message
            << std::endl;
  std::cout << "Login response data size: " << login_response.data.size()
            << std::endl;
  ...
  SimpleLuaActor::LuaMessage move_msg("move", {{"x", "5"}, {"y", "3"}});
  ...
  std::cout
      << "\n🎉 All SimpleLuaActor tests passed! Game logic in Lua is working!"
      << std::endl;
}
