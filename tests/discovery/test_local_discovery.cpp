...
BOOST_AUTO_TEST_CASE(test_register_and_query_single_service) {
    ...
    ServiceInstance instance = {"auth-service", "instance-1", "tcp://127.0.0.1:9001", ServiceMetadata{}, std::chrono::steady_clock::time_point::max()};
...
BOOST_AUTO_TEST_CASE(test_register_multiple_instances_and_query_all) {
    auto discovery = make_local_discovery();
    ServiceInstance instance1 = {"auth-service", "instance-1", "tcp://127.0.0.1:9001", ServiceMetadata{}, std::chrono::steady_clock::time_point::max()};
    ServiceInstance instance2 = {"auth-service", "instance-2", "tcp://127.0.0.1:9002", ServiceMetadata{}, std::chrono::steady_clock::time_point::max()};
    ...
    for (int i = 0; i < num_threads; ++i) {
        threads.emplace_back([&, i]() {
            for (int j = 0; j < services_per_thread; ++j) {
                ...
                std::string instance_id = "instance-" + std::to_string(i) + "-" + std::to_string(j);
                ServiceInstance instance = {service_name, instance_id, "addr", ServiceMetadata{}, std::chrono::steady_clock::time_point::max()};
...
BOOST_AUTO_TEST_CASE(test_ttl_expiration) {
    auto discovery = make_local_discovery();
    ServiceInstance instance1 = {"test-service", "instance-ttl-1", "tcp://127.0.0.1:9001", ServiceMetadata{}, std::chrono::steady_clock::time_point::max()};
    ServiceInstance instance2 = {"test-service", "instance-ttl-2", "tcp://127.0.0.1:9002", ServiceMetadata{}, std::chrono::steady_clock::time_point::max()};
    ...
    ServiceInstance instance3 = {"test-service", "instance-ttl-3", "tcp://127.0.0.1:9003", ServiceMetadata{}, std::chrono::steady_clock::time_point::max()};
    ...
    BOOST_CHECK_EQUAL(discovery->query_all_services("test-service")[0].instance_id, "instance-ttl-2");
}
...
BOOST_AUTO_TEST_CASE(test_persistence) {
    ...
    auto discovery1 = make_local_discovery(std::chrono::seconds(1), test_file_path);

    ServiceInstance instance1 = {"persisted-service", "p-instance-1", "tcp://127.0.0.1:9001", ServiceMetadata{}, std::chrono::steady_clock::time_point::max()};
    ServiceInstance instance2 = {"persisted-service", "p-instance-2", "tcp://127.0.0.1:9002", ServiceMetadata{}, std::chrono::steady_clock::time_point::max()};

    BOOST_CHECK(discovery1->register_service(instance1));
    BOOST_CHECK(discovery1->register_service(instance2));

    BOOST_CHECK_EQUAL(discovery1->query_all_services("persisted-service").size(), 2);
    ...
    auto discovery2 = make_local_discovery(std::chrono::seconds(1), test_file_path);
