...
BOOST_AUTO_TEST_CASE(test_ttl_expiration) {
    ...
    BOOST_CHECK(
        discovery->register_service(instance1, std::chrono::seconds(1)));
    ...
    ServiceInstance instance3 = {"test-service", "instance-ttl-3",
                                 ...
                                 std::chrono::steady_clock::time_point::max()};
    BOOST_CHECK(
        discovery->register_service(instance3, std::chrono::seconds(1)));
    BOOST_CHECK_EQUAL(discovery->query_all_services("test-service").size(), 2);
    std::this_thread::sleep_for(std::chrono::seconds(2));
...
BOOST_AUTO_TEST_CASE(test_persistence) {
    ...
    BOOST_CHECK_EQUAL(
        discovery1->query_all_services("persisted-service").size(), 2);
