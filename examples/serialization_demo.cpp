...
int main() {
    ...
    try {
        ...
        // Deserialize
        auto restored_player = deserialize_as<SerializationFormat::JSON, Player>(json_explicit);
        std::cout << "Restored player name: " << restored_player.name << std::endl;
        ...
        try {
            auto msgpack_data = serialize_as<SerializationFormat::MESSAGEPACK>(player);
            std::cout << "MessagePack serialized, size: " << msgpack_data.size() << " bytes" << std::endl;
            
            auto msgpack_restored = deserialize_as<SerializationFormat::MESSAGEPACK, Player>(msgpack_data);
            std::cout << "MessagePack restored player name: " << msgpack_restored.name << std::endl;
        ...
        switch (recommended_format) {
            case SerializationFormat::JSON: format_name = "JSON"; break;
            case SerializationFormat::PROTOBUF: format_name = "Protobuf"; break;
            case SerializationFormat::MESSAGEPACK: format_name = "MessagePack"; break;
            case SerializationFormat::SPROTO: format_name = "sproto"; break;
            default: break;
        }
        
        std::cout << "Recommended format for Player: " << format_name << std::endl;
        ...
        std::cout << "JSON convenience: " << json_str.substr(0, 50) << "..." << std::endl;
        
        auto convenience_player = from_json_string<Player>(json_str);
        std::cout << "Convenience restored: " << convenience_player.name << std::endl;
