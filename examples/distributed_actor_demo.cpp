...
public:
    PlayerActor(caf::actor_config& cfg, const std::string& name, int level)
        ...
        SHIELD_LOG_INFO << "Player actor created: " << player_name_
                        << " (level " << level_ << ")";
    }

protected:
    caf::behavior make_behavior() override {
        return {[this](player_move& move) {
                    SHIELD_LOG_INFO << "Player " << player_name_
                                    << " moved to (" << move.x << ", " << move.y
                                    << ")";
                    position_x_ = move.x;
                    position_y_ = move.y;
                },
                [this](const std::string& message) {
                    ...
                    return "Player " + player_name_ +
                           " says: Received your message!";
                },
                [this](caf::get_atom) {
                    return std::make_tuple(player_name_, level_, position_x_,
                                           position_y_);
                }};
    }
...
public:
    RoomActor(caf::actor_config& cfg, const std::string& name, int max_players)
        ...
        SHIELD_LOG_INFO << "Room actor created: " << room_name_
                        << " (max players: " << max_players_ << ")";
    }

protected:
    caf::behavior make_behavior() override {
        return {[this](player_join& join) {
                    if (players_.size() < static_cast<size_t>(max_players_)) {
                        players_.push_back(join.player_name);
                        SHIELD_LOG_INFO << "Player " << join.player_name
                                        << " joined room " << room_name_;
                        return std::string("Welcome to room " + room_name_ +
                                           ", " + join.player_name + "!");
                    } else {
                        SHIELD_LOG_WARNING << "Room " << room_name_
                                           << " is full, rejected "
                                           << join.player_name;
                ...
                [this](const std::string& broadcast_msg) {
                    SHIELD_LOG_INFO << "Broadcasting in room " << room_name_
                                    << ": " << broadcast_msg;
                    return "Broadcast sent to " +
                           std::to_string(players_.size()) + " players in " +
                           room_name_;
                }};
    }
    ...
    caf::behavior make_behavior() override {
        return {[this](const std::string& command) {
            ...
            if (command == "list_players") {
                auto players =
                    coordinator_->find_actors_by_type(ActorType::LOGIC);
                std::string result = "Found " + std::to_string(players.size()) +
                                     " player actors: ";
...
void demonstrate_distributed_actors() {
    ...
    auto gateway1 = coordinator1->spawn_and_register<GatewayActor>(
        ActorType::GATEWAY, "gateway_main", "", {{"port", "8080"}},
        coordinator1.get());
    ...
    if (found_player2) {
        SHIELD_LOG_INFO
            << "Node 1 successfully discovered player_bob from node 2";

        // Send message to remote actor
        coordinator1->send_to_actor("player_bob",
                                    std::string("Hello from Alice on node 1!"));
    } else {
    ...
    if (found_room1) {
        SHIELD_LOG_INFO
            << "Node 2 successfully discovered room_tavern from node 1";
    ...
    size_t players_notified = coordinator1->broadcast_to_type(
        ActorType::LOGIC,
        std::string("Server announcement: Maintenance in 5 minutes!"));
    SHIELD_LOG_INFO << "Broadcast sent to " << players_notified
                    << " logic actors";
    ...
    auto game_players =
        coordinator1->get_distributed_system().find_actors_by_group(
            "game_players");
    SHIELD_LOG_INFO << "Found " << game_players.size()
                    << " actors in 'game_players' group:";
    for (const auto& actor : game_players) {
        SHIELD_LOG_INFO << "  - " << actor.metadata.name << " on node "
                        << actor.metadata.node_id;
    }

    auto game_rooms =
        coordinator2->get_distributed_system().find_actors_by_group(
            "game_rooms");
    SHIELD_LOG_INFO << "Found " << game_rooms.size()
                    << " actors in 'game_rooms' group:";
