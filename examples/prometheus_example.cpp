...
public:
    ...
    void start() {
        ...
        player_simulation_thread_ =
            std::thread(&GameServer::simulate_players, this);
        network_simulation_thread_ =
            std::thread(&GameServer::simulate_network, this);
        room_simulation_thread_ =
            std::thread(&GameServer::simulate_rooms, this);

        std::cout << "Game server started with Prometheus monitoring"
                  << std::endl;
        std::cout << "Metrics available at: http://localhost:9090/metrics"
                  << std::endl;
    }
...
private:
    void simulate_players() {
        ...
        while (running_) {
            ...
            if (player_count < 100 &&
                std::uniform_int_distribution<>(0, 10)(rng_) < 3) {
                SHIELD_METRIC_INC_PLAYERS();
                player_count++;
                std::cout << "Player joined. Total: " << player_count
                          << std::endl;
            } else if (player_count > 0 &&
                       std::uniform_int_distribution<>(0, 10)(rng_) < 2) {
                SHIELD_METRIC_DEC_PLAYERS();
                player_count--;
                std::cout << "Player left. Total: " << player_count
                          << std::endl;
            }
    ...
    void simulate_network() {
        while (running_) {
            ...
            size_t bytes_sent =
                std::uniform_int_distribution<size_t>(100, 2000)(rng_);
            size_t bytes_received =
                std::uniform_int_distribution<size_t>(50, 1000)(rng_);
        ...
        while (running_) {
            ...
            if (room_count < 20 &&
                ...
                std::cout << "Room created. Total: " << room_count << std::endl;
                       ...
                       std::uniform_int_distribution<>(0, 10)(rng_) < 1) {
                SHIELD_METRIC_DEC_ROOMS();
                room_count--;
                std::cout << "Room destroyed. Total: " << room_count
                          << std::endl;
            }
