...
// consumer thread that loops XREADGROUP. For C-ABI consumers, messages are
// dispatched to the registered callback; on callback return, XACK is called
// automatically. For Lua subscribers, the consumer thread never touches
// Lua: each message is posted to the owning service actor via
// host_api.lua_post_to_service, and the Lua callback + XACK run on the
// actor's dispatch context. Messages that cannot be posted stay pending
// (at-least-once).
//
...
void consumer_loop(shield_queue_conn* c, const std::string& channel,
    ...
    while (c->running.load()) {
        try {
            // XREADGROUP GROUP <group> <consumer> COUNT <n> BLOCK <ms> STREAMS
            // <stream> >
            auto result = c->redis->command(
                "XREADGROUP", "GROUP", kGroupName, consumer_name, "COUNT",
                std::to_string(kReadCount), "BLOCK", std::to_string(kBlockMs),
                "STREAMS", channel, ">");
            ...
            for (size_t si = 0; si < result->elements; ++si) {
                ...
                for (size_t ei = 0; ei < entries->elements; ++ei) {
                    ...
                    // entry->element[0] = message id, entry->element[1] =
                    // fields array
...
const shield_queue_v1& queue_vtable() {
    static const shield_queue_v1 v = {
        ...
        "1.0.0",
        // connect
        [](const shield_queue_config* cfg, char* err_buf,
           int err_buf_size) -> shield_queue_conn* {
            ...
            try {
                ...
                opts.socket_timeout = std::chrono::milliseconds(
                    cfg->command_timeout_ms > 0 ? cfg->command_timeout_ms
                                                : 2000);
        ...
        [](shield_queue_conn* c, const char* channel, const char* data,
           int data_len) -> int {
            if (!c || !c->redis || !channel || !data) return -1;
            try {
                std::string msg(data,
                                data_len > 0 ? data_len : std::strlen(data));
...
struct lua_subscription {
    ...
    // Owning service actor. The consumer thread never touches Lua; messages
    // are posted to this service via host_api.lua_post_to_service and the
    // Lua callback runs on the service actor's dispatch context.
...
struct queue_instance {
    ...
    void stop_all() {
        std::unordered_map<std::string, std::shared_ptr<lua_subscription>>
            snapshot;
        {
...
sol::table make_instance_proxy(sol::state_view lua, queue_instance* inst) {
    ...
    proxy.set_function(
        ...
        [inst](sol::this_state s, std::string channel,
            ...
            // The Lua callback must run on the owning service actor, so the
            // owner has to be known at subscribe time.
            const char* owner = nullptr;
            if (inst->host_api && inst->host_api->lua_current_service_id) {
            ...
            shield_plugin_context_v1* ctx = inst->ctx;
            sub->worker = std::thread([sub_ptr, host_api, ctx]() {
                // Posted delivery: runs on the owning service actor via
                // host_api.lua_post_to_service. The Lua callback and the
                // XACK both execute on the actor thread; the consumer thread
                // only does XREADGROUP. Messages that cannot be posted
                // (service gone) stay pending in Redis (at-least-once).
                struct Delivery {
                    std::weak_ptr<lua_subscription> sub;
                    std::string msg_id;
                    std::string payload;
                };
...
// shield.pool.stats.v1 — queue.redis opens one fresh sw::redis::Redis
// connection per operation and keeps no persistent pool, so every gauge is
// -1 (unknown); reporting a fabricated pool shape would mislead dashboards.
int queue_pool_get_stats(struct shield_plugin_instance_v1* self,
...
const shield_pool_stats_v1& queue_pool_stats_vtable() {
    static const shield_pool_stats_v1 v{
        sizeof(shield_pool_stats_v1),
        &queue_pool_get_stats,
    };
    return v;
}
...
int queue_create(const shield_plugin_create_args_v1* args,
    ...
    inst->shell.get_interface = [](shield_plugin_instance_v1*,
        ...
        if (iface && std::strcmp(iface, SHIELD_POOL_STATS_INTERFACE) == 0)
            return &queue_pool_stats_vtable();
        return nullptr;
    };
