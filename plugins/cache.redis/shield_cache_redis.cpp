...
//
// v1 ABI (shield_plugin_get_v1). Dual-path connection model (Phase 2 of the
// redis.driver migration): when the instance is wired to a redis.driver
// instance via manifest `requires` + `plugins.instances[].dependencies`, all
// operations go through the driver's shield.redis.v1 vtable on that
// instance's shared connection pool. When no dependency is wired, each
// operation falls back to a self-owned sw::redis::Redis connection built
// from this instance's own config.
//
...
std::shared_ptr<sw::redis::Redis> open_redis(const std::string& host, int port,
    ...
    try {
        ...
        pool_opts.size =
            pool_size > 0 ? static_cast<std::size_t>(pool_size) : 8;
...
const shield_cache_v1& cache_vtable() {
    static const shield_cache_v1 v = {
        ...
        "1.0.0",
        // connect
        [](const shield_cache_config* cfg, char* err_buf,
           int err_buf_size) -> shield_cache_conn* {
            ...
            std::string host =
                cfg->host && cfg->host[0] ? cfg->host : "localhost";
        ...
        // set
        [](shield_cache_conn* c, const char* key, const char* value,
           int ttl_seconds) -> int {
        ...
        [](shield_cache_conn* c, const char* key) -> int {
            ...
            try {
                return c->redis->exists(std::string(key)) > 0 ? 1 : 0;
            } catch (...) {
                return 0;
            }
        },
        ...
        [](shield_cache_conn* c, const char* key, int64_t* out) -> int {
            ...
            try {
                *out = c->redis->incr(std::string(key));
                return 0;
            } catch (...) {
                return -1;
            }
        ...
        // incr_by
        [](shield_cache_conn* c, const char* key, int64_t amount,
           int64_t* out) -> int {
            if (!c || !c->redis || !key || !out) return -1;
            try {
                *out = c->redis->incrby(std::string(key), amount);
                return 0;
            } catch (...) {
                return -1;
            }
        ...
        // hget
        [](shield_cache_conn* c, const char* key, const char* field,
           shield_cache_value* out) -> int {
    ...
    proxy.set_function(
        ...
        [inst](sol::this_state s, std::string key,
            ...
            if (inst->redis_driver && inst->redis_handle) {
                std::string k = key;
                std::string amt =
                    std::to_string(static_cast<long long>(amount));
                shield_redis_arg_v1 args[] = {{"INCRBY", 6},
                                              {k.data(), k.size()},
                                              {amt.data(), amt.size()}};
                shield_redis_value_v1* out = nullptr;
                shield_error_v1 err{};
